...
interface ApiHomeEvent {
  ...
  /** True for events generated by the Football Intelligence Engine (FIE 2.0) */
  isFie?: boolean;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const eventsRaw = await prisma.event.findMany({
      ...
      include: {
        _count: { select: { Prediction: true, Trade: true, feedbacks: true } },
        ammState: {
    ...
    const processed = await Promise.all(
      eventsRaw.map(async (event) => {
        ...
        return {
          ...
          hasFeedback: ((event._count as { feedbacks?: number }).feedbacks ?? 0) > 0,
          isFie:
            !!(
              (event as { creationMetadata?: unknown }).creationMetadata &&
              typeof (event as { creationMetadata?: unknown }).creationMetadata === "object" &&
              "fie_version" in
                ((event as { creationMetadata?: unknown }).creationMetadata as object)
            ),
        };
      })
    );
