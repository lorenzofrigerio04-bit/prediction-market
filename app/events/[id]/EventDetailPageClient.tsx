...
import type { EventDetailApiPayload } from "@/lib/events/get-event-detail-payload";

const EventProbabilityChart = dynamic(
  () => import("@/components/events/EventProbabilityChart"),
  {
    ssr: false,
    loading: () => (
      <div
        className="h-[220px] w-full animate-pulse rounded-xl border border-border/50 bg-white/[0.03]"
        aria-hidden
      />
    ),
  }
...
export default function EventDetailPageClient({
  ...
  eventId: string;
  initialPayload: EventDetailApiPayload;
}) {
  ...
  const [event, setEvent] = useState<EventDetail | null>(
    () => initialPayload.event as unknown as EventDetail
  );
  const [userPosition, setUserPosition] = useState<UserPosition | null>(
    initialPayload.userPosition ?? null
  );
  ...
  const [isFollowing, setIsFollowing] = useState(initialPayload.isFollowing ?? false);
  ...
  const skipDuplicateEventFetch = useRef(true);
  useEffect(() => {
    if (!eventId) return;
    if (skipDuplicateEventFetch.current) {
      skipDuplicateEventFetch.current = false;
    } else {
      fetchEvent();
    }
    if (session?.user?.id) {
