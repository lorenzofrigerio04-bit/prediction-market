...
export default function CreaEventTileConfigurator({
  ...
  const fallbackGradient = category ? getCategoryFallbackGradient(category) : "linear-gradient(135deg, rgb(var(--surface)) 0%, rgb(var(--bg)) 100%)";
  ...
  return (
    <div className="crea-event-tile-configurator group relative block min-h-[175px] overflow-hidden rounded-lg border-0 bg-transparent transition-all duration-300 sm:min-h-[195px]">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-bg/95 via-bg/75 to-bg/45 pointer-events-none" aria-hidden />
      <div className="relative z-10 flex h-full min-h-[175px] flex-col justify-between p-3 sm:min-h-[195px] sm:p-4">
        ...
        <div ref={categoryRef} className="relative flex flex-col items-start gap-1">
          {readOnly ? (
            <span
              className="inline-flex w-fit rounded-md border border-border/80 bg-bg/80 px-2 py-0.5 text-xs font-semibold text-fg shadow-[0_2px_6px_rgb(0_0_0_/_0.7)] backdrop-blur-sm sm:text-ds-micro"
            ...
            <>
              <button
                ...
                className="inline-flex w-fit rounded-md border border-border/80 bg-bg/80 px-2 py-0.5 text-xs font-semibold text-fg shadow-[0_2px_6px_rgb(0_0_0_/_0.7)] backdrop-blur-sm sm:text-ds-micro hover:bg-bg/90 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-transparent"
              ...
              {categoryOpen && (
                <div
                  className="absolute left-0 top-full z-50 mt-1 min-w-[160px] py-1.5 rounded-xl border border-border/70 bg-bg/90 backdrop-blur-md shadow-lg max-h-[min(280px,70vh)] overflow-y-auto"
                  role="listbox"
                    ...
                    return (
                      <button
                        ...
                        className={`w-full px-3 sm:px-4 py-2.5 text-left text-sm sm:text-ds-body-sm font-medium transition-colors ${
                          isSelected ? "bg-primary/15 text-primary" : "text-fg hover:bg-surface/80"
                        }`}
                      >
                        {opt}
                      </button>
        ...
        <div>
          {readOnly ? (
            <>
              <p
                className="mb-2 w-full text-sm font-semibold leading-snug text-fg sm:text-ds-body-sm"
                style={{ textShadow: "0 2px 8px rgba(0,0,0,1), 0 0 1px rgba(0,0,0,1), 0 1px 3px rgba(0,0,0,0.9)" }}
              >
                {title || "—"}
              </p>
              <p
                className="mb-2 w-full text-xs font-medium text-fg/90 sm:text-ds-micro line-clamp-2"
                style={{ textShadow: "0 1px 4px rgba(0,0,0,1), 0 0 1px rgba(0,0,0,0.8)" }}
              >
                {description || "—"}
              </p>
              <div
                className="flex h-1.5 w-full overflow-hidden rounded-full bg-bg/60 shadow-inner backdrop-blur-[1px]"
                role="presentation"
              >
                <div
              ...
              {readOnly && closesAt && (
                <div
                  className="mt-2 flex items-center gap-2 text-xs font-medium text-fg/90"
                  style={{ textShadow: "0 1px 3px rgba(0,0,0,0.8)" }}
                >
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-success/90 text-bg" aria-hidden>
            ...
            <>
              ...
              <input
                ...
                placeholder="Titolo"
                className="mb-2 w-full bg-transparent text-sm font-semibold leading-snug text-fg placeholder:text-fg-subtle focus:outline-none sm:text-ds-body-sm"
                style={{ textShadow: "0 2px 8px rgba(0,0,0,1), 0 0 1px rgba(0,0,0,1), 0 1px 3px rgba(0,0,0,0.9)" }}
              />
              ...
              <textarea
                ...
                rows={2}
                className="mb-2 w-full resize-none bg-transparent text-xs font-medium text-fg placeholder:text-fg-subtle focus:outline-none sm:text-ds-micro"
                style={{ textShadow: "0 1px 4px rgba(0,0,0,1), 0 0 1px rgba(0,0,0,0.8)" }}
              />
              <div
                className="flex h-1.5 w-full overflow-hidden rounded-full bg-bg/60 shadow-inner backdrop-blur-[1px]"
