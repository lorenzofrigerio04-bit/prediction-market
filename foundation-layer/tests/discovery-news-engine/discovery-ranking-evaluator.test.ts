...
import {
  ...
  createDiscoveryTrendSnapshotId,
  createDiscoveryTrendIndicatorSet,
  ...
  DiscoveryTrendHorizon,
  DiscoveryTrendStatus,
  DiscoverySignalKind,
...
function signal(opts: {
  ...
  sourceKey: ReturnType<typeof createDiscoverySourceKey>;
  sourceRole: DiscoverySourceUsageRole | null;
}) {
...
function snapshot(opts: {
  clusterId: ReturnType<typeof createDiscoveryStoryClusterId>;
...
describe("discoveryRankingEvaluator", () => {
  it("high-priority cluster with strong Italian + authoritative/editorial support", () => {
    ...
    const summariesByClusterId = new Map<string, ReturnType<typeof createDiscoveryStoryClusterSummary>>();
    summariesByClusterId.set(String(cluster.clusterId), summaryWithGeo);
    const snapshotsByClusterId = new Map<string, readonly ReturnType<typeof createDiscoveryTrendSnapshot>[]>();
    snapshotsByClusterId.set(String(cluster.clusterId), [] as const);
    ...
    const authOrEditorialReason = entry.reasons.find(
      (r) => r.code === "authoritative_source" || r.code === "editorial_source",
    );
  ...
  it("attention-heavy cluster with weak authoritative support ranks lower and has caution flag", () => {
    ...
    expect(result.entries.length).toBe(1);
    const entry = result.entries[0]!;
    expect([DiscoveryPriorityClass.MEDIUM, DiscoveryPriorityClass.LOW, DiscoveryPriorityClass.INSUFFICIENT_EVIDENCE]).toContain(
      entry.priorityClass,
    );
    ...
    const hasCaution =
      (entry.cautionFlags ?? []).includes("high_attention_low_authoritative") ||
  ...
  it("low-information / insufficient evidence gets LOW or INSUFFICIENT_EVIDENCE", () => {
    ...
    expect(result.entries.length).toBe(1);
    const entry = result.entries[0]!;
    expect([DiscoveryPriorityClass.LOW, DiscoveryPriorityClass.INSUFFICIENT_EVIDENCE]).toContain(
      entry.priorityClass,
    );
