...
import {
  ...
  createDiscoveryRankingEntry,
  createDiscoveryRankingScoreBreakdown,
...
function rankingEntry(opts: {
  ...
  breakdown: ReturnType<typeof createDiscoveryRankingScoreBreakdown>;
  reasons?: readonly ReturnType<typeof createDiscoveryRankingReason>[];
...
function breakdown(overrides: Partial<ReturnType<typeof createDiscoveryRankingScoreBreakdown>> = {}) {
...
describe("eventLeadExtractionEvaluator", () => {
  ...
  it("attention-heavy but weakly supported cluster does not produce EventLead", () => {
    ...
    if (decision.outcome === "not_extracted") {
      ...
      const hasRelevantReason =
        decision.reasons.some((r) => r.code === "attention_only_no_authoritative") ||
  ...
  it("explainability fields are populated on both positive and negative decisions", () => {
    ...
    const noLeadDecision = result.decisions.find(
      (d) => d.outcome === "not_extracted" && d.clusterId === clusterIdNoLead,
    );
