...
describe("missing schema fail-fast", () => {
  ...
  const initialTransition = createInitialWorkflowTransition(
    createTimestamp("2026-03-01T00:00:00.000Z"),
  );
  ...
  it("throws when required validator schemas are missing", () => {
    expectFailFast(SOURCE_RECORD_SCHEMA_ID, sourceRecordSchema, () =>
      validateSourceRecord(sourceRecord),
    );
    expectFailFast(EVENT_SIGNAL_SCHEMA_ID, eventSignalSchema, () =>
      validateEventSignal(eventSignal),
    );
    expectFailFast(CANONICAL_EVENT_SCHEMA_ID, canonicalEventSchema, () =>
      validateCanonicalEvent(canonicalEvent),
    );
    expectFailFast(STRUCTURED_CLAIM_SCHEMA_ID, structuredClaimSchema, () =>
      validateStructuredClaim(structuredClaim),
    );
    expectFailFast(MARKET_OUTCOME_SCHEMA_ID, marketOutcomeSchema, () =>
      validateMarketOutcome(outcomeYes),
    );
    expectFailFast(CANDIDATE_MARKET_SCHEMA_ID, candidateMarketSchema, () =>
      validateCandidateMarket(candidateMarket),
    );
    expectFailFast(WORKFLOW_INSTANCE_SCHEMA_ID, workflowInstanceSchema, () =>
      validateWorkflowInstance(workflowInstance),
    );
    expectFailFast(VALIDATION_REPORT_SCHEMA_ID, validationReportSchema, () =>
