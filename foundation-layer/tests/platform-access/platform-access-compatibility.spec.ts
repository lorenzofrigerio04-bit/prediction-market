...
import {
  createPlatformActionCompatibility,
  type PlatformActionCompatibility,
} from "../../src/platform-access/compatibility/entities/platform-action-compatibility.entity.js";
...
const makeCompatibility = (overrides?: Partial<PlatformActionCompatibility>) =>
  createPlatformActionCompatibility({
...
describe("platform-access compatibility requirements", () => {
  ...
  it("17 - keeps PLATFORM_ACCESS compatibility on GLOBAL scope typing", () => {
    ...
    expect(validatePlatformActionCompatibility(entry as PlatformActionCompatibility).isValid).toBe(true);
  });

  it("18 - rejects PLATFORM_ACCESS compatibility with ENTITY scope", () => {
    const report = validatePlatformActionCompatibility(
      makeCompatibility({
