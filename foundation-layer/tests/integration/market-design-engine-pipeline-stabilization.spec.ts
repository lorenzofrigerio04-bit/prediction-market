...
const createRulebookCompiler = () =>
  new DeterministicRulebookCompiler(
...
describe("Market Design Engine pipeline stabilization", () => {
  ...
  it("pipeline with invalid event fails in event-intelligence validation and does not publish", () => {
    ...
    const eventReport = validateCanonicalEvent(invalidCanonicalEvent as typeof pipeline.canonical_event);
    expect(eventReport.isValid).toBe(false);
    expect(eventReport.issues.some((issue) => issue.code === "MISSING_SUPPORTING_OBSERVATIONS")).toBe(
      true,
    );
  ...
  it("pipeline with missing deadline fails at deadline validation and blocks flow", () => {
    ...
    const deadlineReport = validateDeadlineResolution(invalidDeadlineResolution as never);
    ...
    const pipelineReport = validateMarketDraftPipeline(invalidPipeline as never);
    expect(pipelineReport.isValid).toBe(false);
    expect(
      pipelineReport.issues.some(
        (issue) =>
          issue.path === "/deadline_resolution/event_deadline" || issue.path === "/deadline_resolution",
      ),
    ).toBe(true);
  });
  ...
  it("pipeline with invalid outcome generation fails deterministically", () => {
    ...
    const outcomeReport = validateOutcomeGenerationResult(invalidOutcomeResult as never);
    expect(outcomeReport.isValid).toBe(false);
    expect(outcomeReport.issues.some((issue) => issue.code === "INVALID_BINARY_CARDINALITY")).toBe(
      true,
    );
  ...
  it("pipeline with invalid rulebook section is rejected by publishing validation", () => {
    ...
    const invalidCompilation = {
      ...
      included_sections: validCompilation.included_sections.filter(
        (section) => section.section_type !== RulebookSectionType.TIME_POLICY,
      ),
    };

    const compilationReport = validateRulebookCompilation(invalidCompilation as never);
    expect(compilationReport.isValid).toBe(false);
    expect(
      compilationReport.issues.some((issue) => issue.code === "MISSING_REQUIRED_RULEBOOK_SECTION"),
    ).toBe(true);
    ...
    const publishableReport = validatePublishableCandidate(candidate, {
      linked_artifacts: {
        title_set_nullable: titleSet,
        resolution_summary_nullable: resolutionSummary,
        rulebook_compilation_nullable: invalidCompilation as never,
      },
    });
    expect(publishableReport.isValid).toBe(false);
    expect(
      publishableReport.issues.some((issue) => issue.code === "MISSING_REQUIRED_RULEBOOK_SECTION"),
    ).toBe(true);
  });
});
