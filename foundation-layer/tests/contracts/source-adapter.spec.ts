...
import type { SourceAdapter } from "@/sources/interfaces/source-adapter.js";
import {
  AuthorityLevel,
  EnablementStatus,
  IdentifierKind,
  ...
  validateObservationNormalizationResult,
} from "@/index.js";
...
import type { FetchRawRequest } from "@/sources/interfaces/fetch-raw-request.js";
import type { FetchRawResult } from "@/sources/interfaces/fetch-raw-result.js";
import type { PayloadValidationResult } from "@/sources/interfaces/payload-validation-result.js";
import type { ObservationNormalizationResult } from "@/observations/interfaces/observation-normalization-result.js";

type FakeRawPayload = Readonly<{
  headline: string;
  body: string;
  observedAt: string;
  reference: string;
...
const createSourceDefinitionForAdapter = (parseStrategy: ParseStrategy) =>
  createSourceDefinition({
...
const buildObservationFromPayload = (
...
const fakeAdapter: SourceAdapter<FakeRawPayload> = {
  canHandle(sourceDefinition) {
    return sourceDefinition.parseStrategy === ParseStrategy.SEMI_STRUCTURED_FEED;
  },
  async fetchRaw(_request: FetchRawRequest): Promise<FetchRawResult<FakeRawPayload>> {
    return {
      payload: {
        headline: "Headline",
        body: "Body",
  ...
  validatePayload(payload: FakeRawPayload): PayloadValidationResult {
  ...
  normalize(payload, sourceDefinition): ObservationNormalizationResult {
...
describe("source adapter contract", () => {
  it("canHandle behavior on SourceAdapter", () => {
    expect(fakeAdapter.canHandle(createSourceDefinitionForAdapter(ParseStrategy.SEMI_STRUCTURED_FEED))).toBe(
      true,
    );
    expect(fakeAdapter.canHandle(createSourceDefinitionForAdapter(ParseStrategy.STRUCTURED_API))).toBe(
      false,
    );
  });
  ...
  it("SourceAdapter contract behavior", async () => {
    ...
    const normalization = fakeAdapter.normalize(
      fetchResult.payload,
      sourceDefinition,
      {
        requestedAt: createTimestamp("2026-03-01T00:00:00.000Z"),
  ...
  it("normalization result validation", () => {
    ...
    const report = validateObservationNormalizationResult(invalidResult as never);
