...
describe("ResolutionSummary", () => {
  it("accepts a valid summary", () => {
    const summary = createResolutionSummary({
      ...
      one_line_resolution_summary:
        "Resolves Yes if the defined condition is satisfied before deadline on top-priority sources.",
      summary_basis: {
  ...
  it("validator rejects structurally underspecified basis", () => {
    ...
    expect(
      report.issues.some(
        (issue) =>
          issue.code === "INVALID_RESOLUTION_RULE_REFERENCE" ||
          issue.code === "INVALID_SOURCE_HIERARCHY_REFERENCE" ||
          issue.code === "INVALID_DEADLINE_REFERENCE" ||
          issue.code === "INSUFFICIENT_SUMMARY_BASIS_POINTS",
      ),
    ).toBe(true);
  });
});
