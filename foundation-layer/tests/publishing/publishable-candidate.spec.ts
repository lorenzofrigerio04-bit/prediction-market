...
describe("PublishableCandidate", () => {
  ...
  it("invalid publishable candidate with missing linked artifacts", () => {
    const invalid = createPublishableCandidate({
      id: "pcnd_invalidcand1" as never,
      version: createEntityVersion(1),
      market_draft_pipeline_id: "mdp_validpipe001" as never,
      title_set_id: "bad-title-id" as never,
      resolution_summary_id: "bad-summary-id" as never,
      rulebook_compilation_id: "bad-rulebook-id" as never,
  ...
  it("validator rejects candidate when linked rulebook misses required section", () => {
    ...
    const compilation = new DeterministicRulebookCompiler(
      new DeterministicTimePolicyRenderer(),
      new DeterministicSourcePolicyRenderer(),
      new DeterministicEdgeCaseRenderer(),
    ).compile({ pipeline });
    ...
    const report = validatePublishableCandidate(candidate, {
      linked_artifacts: {
        ...
        rulebook_compilation_nullable: {
          ...compilation,
          included_sections: compilation.included_sections.filter(
