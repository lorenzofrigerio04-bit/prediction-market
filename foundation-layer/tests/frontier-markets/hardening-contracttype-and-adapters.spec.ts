...
const makeRaceDefinition = (status: RaceValidationStatus, activeSecond = true) =>
...
describe("Frontier hardening: ContractType and adapters", () => {
  ...
  it("blocks promotion when validation_status is not ready", () => {
    ...
    const marketDraftResult = new MarketDraftPipelineCompatibilityAdapter().adapt(
      validRace,
      invalidValidationReport,
    );
    const publishingResult = new PublishingEngineCompatibilityAdapter().adapt(
      validRace,
      invalidValidationReport,
    );
    const editorialResult = new EditorialPipelineCompatibilityAdapter().adapt(
      validRace,
      invalidValidationReport,
    );

    expect(marketDraftResult.status).toBe(AdvancedCompatibilityStatus.COMPATIBLE_WITH_WARNINGS);
    expect(marketDraftResult.mapped_artifact["readiness"]).toBe(
    ...
    expect(publishingResult.mapped_artifact["readiness"]).toBe(
      AdvancedCompatibilityStatus.COMPATIBLE_WITH_WARNINGS,
    );
    expect(editorialResult.status).toBe(AdvancedCompatibilityStatus.COMPATIBLE_WITH_WARNINGS);
    expect(editorialResult.mapped_artifact["needs_manual_review"]).toBe(true);
