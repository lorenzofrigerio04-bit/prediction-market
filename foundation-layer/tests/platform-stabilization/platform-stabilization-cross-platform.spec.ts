...
import {
  ActionKey,
  DeterministicPlatformCompatibilityAdapter,
  TargetModule,
  validatePlatformActionCompatibility,
} from "../../src/platform-access/index.js";
import {
  PlatformAccessConsoleCompatibilityAdapter,
  validateOperationsConsoleCompatibility,
} from "../../src/operations-console/index.js";
import {
  createGovernanceMetadata,
  createModuleKey,
  createOperationKey,
  PlatformAccessAdminGovernanceCompatibilityAdapter,
} from "../../src/admin-governance/index.js";
import {
  ConsistencyStatus,
  createActionKey,
  createNote,
  createCreditsCompatibilityView,
  createOwnerRef,
  createRelatedRef,
  OperationsConsoleCreditsCompatibilityAdapter,
  validateCreditsCompatibility,
...
describe("platform stabilization cross-module integration", () => {
  it("A - access allowed + governance clear + credits sufficient => action available in console surface", () => {
    ...
    const consoleResult = new PlatformAccessConsoleCompatibilityAdapter().adapt({
      ...
      visibility: "visible",
      allowed_actions: governanceView.allowed_operations.map((operation) => operation as string),
      denied_actions: governanceView.denied_operations.map((operation) => operation as string),
    });

    expect(validateCreditsCompatibility(creditsView, undefined, creditsContext).isValid).toBe(true);
    expect(
      validateOperationsConsoleCompatibility(consoleResult, undefined, {
  ...
  it("C - credits/quota denied action is surfaced as denied in console contract", () => {
    ...
    const consoleResult = new PlatformAccessConsoleCompatibilityAdapter().adapt({
      ...
      visibility: "partial",
      allowed_actions: creditsDeniedView.allowed_actions.map((action) => action as string),
      denied_actions: ["publish"],
    });
