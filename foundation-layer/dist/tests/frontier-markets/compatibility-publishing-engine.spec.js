...
import { createDisplayLabel, createSemanticDefinition, } from "../../src/frontier-markets/value-objects/frontier-text.vo.js";
...
const makeRaceContract = () => createRaceMarketDefinition({
    id: createRaceMarketDefinitionId("frc_frontier003"),
    version: createEntityVersion(1),
    parent_canonical_event_id_nullable: createCanonicalEventIntelligenceId("cevt_frontier006"),
    race_targets: [
        createRaceTarget({
    ...
    deadline_resolution: createDeadlineResolution({
        id: createDeadlineResolutionId("dlr_frontier05"),
        canonical_event_id: createCanonicalEventIntelligenceId("cevt_frontier006"),
    ...
    source_hierarchy_selection: createSourceHierarchySelection({
        id: createSourceHierarchySelectionId("shs_frontier006"),
        canonical_event_id: createCanonicalEventIntelligenceId("cevt_frontier006"),
...
describe("PublishingEngineCompatibilityAdapter", () => {
    it("compatibility test toward Publishing Engine candidate generation", () => {
        const adapter = new PublishingEngineCompatibilityAdapter();
        const result = adapter.adapt(makeRaceContract());
        const report = validateFrontierMarketCompatibility(result);
        expect(report.isValid).toBe(true);
        expect(result.target).toBe("publishing_engine");
    });
});
//# sourceMappingURL=compatibility-publishing-engine.spec.js.map