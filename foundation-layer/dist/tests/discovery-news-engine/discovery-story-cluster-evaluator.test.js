...
import { discoveryStoryClusterEvaluator, createNormalizedDiscoveryItem, createNormalizedSourceReference, createDiscoverySourceId, createDiscoverySourceKey, createDiscoveryStoryClusterId, createDiscoveryStoryCluster, buildDiscoveryStoryClusterSummary, DiscoveryStoryClusterStatus, DiscoveryStoryMembershipReason, DiscoveryDedupeEvidenceStrength, DiscoveryTopicScope, DiscoveryGeoScope, } from "../../src/discovery-news-engine/index.js";
...
describe("discoveryStoryClusterEvaluator", () => {
    describe("assignToClusters", () => {
        it("creates new cluster when no strong match exists", () => {
            ...
            const { clusters, decisions } = discoveryStoryClusterEvaluator.assignToClusters({ items: [a, b] }, {});
        ...
        it("joins cluster by title + narrow temporal proximity (same day)", () => {
            ...
            const { clusters, decisions } = discoveryStoryClusterEvaluator.assignToClusters({ items: [a, b] }, {});
            expect(clusters.length).toBe(1);
            expect(decisions[1].reason).toBe(DiscoveryStoryMembershipReason.TITLE_AND_TEMPORAL_PROXIMITY);
        ...
        it("explainability: each decision has reason, matchedMemberIdNullable, evidenceStrength, createdNewCluster", () => {
            ...
            const { decisions } = discoveryStoryClusterEvaluator.assignToClusters({ items: [a, b] }, {});
        ...
        it("uses existing clusters when provided in context", () => {
            ...
            const { clusters, decisions } = discoveryStoryClusterEvaluator.assignToClusters({ items: [newItem] }, {
...
describe("buildDiscoveryStoryClusterSummary", () => {
    it("computes member count, source diversity, time span, representative headline", () => {
        ...
        const { clusters } = discoveryStoryClusterEvaluator.assignToClusters({ items: [a, b] }, {});
...
//# sourceMappingURL=discovery-story-cluster-evaluator.test.js.map