...
import { createDiscoverySourceId, createDiscoverySourceKey, createDiscoverySourceDefinition, validateDiscoverySourceDefinition, DiscoverySourceKind, DiscoverySourceTier, DiscoverySourceStatus, DiscoveryPollingHint, DiscoveryGeoScope, DiscoveryTopicScope, DiscoveryTrustTier, DiscoverySourceAuthMode, } from "../../src/discovery-news-engine/index.js";
...
import { createNormalizedSourceReference, createDiscoveryRunId, createDiscoverySignalId, createDiscoverySignalTimeWindow, createDiscoverySignalEvidenceRef, createDiscoveryProvenanceMetadata, createNormalizedDiscoveryItem, createNormalizedDiscoveryPayload, createDiscoverySignal, validateNormalizedDiscoveryPayload, validateDiscoverySignal, DiscoverySignalKind, DiscoverySignalStatus, DiscoverySignalFreshnessClass, DiscoverySignalPriorityHint, DiscoveryEvidenceRole, } from "../../src/discovery-news-engine/index.js";
import { createDiscoveryErrorReport, createDiscoveryValidationFailure, validateDiscoveryErrorReport, DiscoveryErrorCode, } from "../../src/discovery-news-engine/index.js";
import { createDiscoveryRunDefinition, createDiscoveryRunMetrics, createDiscoveryRunResult, createDiscoveryJobDefinition, createDiscoveryJobExecutionRequest, createDiscoveryJobExecutionResult, createDiscoveryJobId, validateDiscoveryRunDefinition, validateDiscoveryJobDefinition, DiscoveryRunTrigger, DiscoveryRunStatus, } from "../../src/discovery-news-engine/index.js";
...
describe("Discovery News Engine Foundation", () => {
    ...
    describe("invalid source definition", () => {
        it("returns validation issues for missing endpoint url", () => {
            ...
            const report = validateDiscoverySourceDefinition(defWithEmptyUrl);
        ...
        it("rejects invalid enum values via schema", () => {
            ...
            const report = validateDiscoverySourceDefinition(defWithInvalidKind);
            expect(report.isValid).toBe(false);
        });
    });
    ...
    describe("invalid normalized payload", () => {
        it("rejects empty items via invariant", () => {
            ...
            expect(() => createNormalizedDiscoveryPayload({
                items: [],
                provenanceMetadata: createDiscoveryProvenanceMetadata({
        ...
        it("returns validation issues for invalid payload shape", () => {
            ...
            const report = validateNormalizedDiscoveryPayload(invalidPayload);
            expect(report.isValid).toBe(false);
        });
    });
...
//# sourceMappingURL=discovery-foundation.test.js.map