...
import { discoverySourceRegistry, catalogEntryToSourceDefinition, createDiscoveryFetchRequest, runConnectorWithNormalize, getAdapterByKey, getSupportedSourceKeys, getAdaptersByKind, ansaRssSourceAdapter, istatNormalizationAdapter, ingvNormalizationAdapter, wikimediaPageviewsNormalizationAdapter, youtubeDataItNormalizationAdapter, googleTrendsItNormalizationAdapter, createDiscoveryFetchedPayload, createDiscoveryTransportMetadata, createDiscoverySourceDefinition, createDiscoverySourceId, createDiscoverySourceKey, createDiscoverySourceEndpoint, DiscoverySourceKind, DiscoveryContentType, DiscoverySourceStatus, DiscoveryPollingHint, DiscoveryGeoScope, DiscoveryTopicScope, DiscoveryTrustTier, DiscoverySourceAuthMode, DiscoverySourceTier, } from "../../src/discovery-news-engine/index.js";
...
describe("ISTAT adapter – normalization behavior", () => {
    ...
    it("throws on empty or missing items array", () => {
        ...
        expect(() => istatNormalizationAdapter.normalize(emptyPayload, { sourceDefinition: istatDef, runIdNullable: null })).toThrow(/No items to normalize/);
    });
});
describe("INGV adapter – normalization behavior", () => {
    ...
    it("throws on empty or missing events array", () => {
        ...
        expect(() => ingvNormalizationAdapter.normalize(emptyPayload, { sourceDefinition: ingvDef, runIdNullable: null })).toThrow(/No items to normalize/);
    });
});
describe("Wikimedia Pageviews adapter", () => {
    ...
    it("throws on empty or missing items array", () => {
        ...
        expect(() => wikimediaPageviewsNormalizationAdapter.normalize(emptyPayload, {
            sourceDefinition: wikimediaDef,
            runIdNullable: null,
...
describe("YouTube Data API (Italy) adapter", () => {
    ...
    it("throws on empty or missing items array", () => {
        ...
        expect(() => youtubeDataItNormalizationAdapter.normalize(emptyPayload, {
            sourceDefinition: youtubeDef,
            runIdNullable: null,
        })).toThrow(/No items to normalize/);
    });
...
describe("Google Trends Italy adapter (experimental)", () => {
    ...
    it("optional/degraded: throws on empty payload", () => {
        ...
        expect(() => googleTrendsItNormalizationAdapter.normalize(emptyPayload, {
            sourceDefinition: googleTrendsDef,
            runIdNullable: null,
        })).toThrow(/No items to normalize/);
    });
...
//# sourceMappingURL=source-adapters.test.js.map