...
import { createDiscoverySourceId, createDiscoverySourceKey, createDiscoverySourceCatalogEntry, catalogEntryToSourceDefinition, validateDiscoverySourceCatalogEntry, createDiscoverySourceRegistry, discoverySourceRegistry, INITIAL_DISCOVERY_SOURCE_CATALOG, DiscoverySourceKind, DiscoverySourceTier, DiscoverySourceStatus, DiscoverySourceUsageRole, DiscoverySourceCapability, DiscoveryGeoScope, DiscoveryTopicScope, DiscoveryTrustTier, DiscoveryPollingHint, DiscoverySourceAuthMode, } from "../../src/discovery-news-engine/index.js";
...
describe("Discovery Source Registry", () => {
    ...
    describe("invalid catalog entry", () => {
        it("returns validation issues for empty name", () => {
            ...
            const report = validateDiscoverySourceCatalogEntry(entryWithEmptyName);
        ...
        it("returns validation issues for invalid role (schema)", () => {
            ...
            const report = validateDiscoverySourceCatalogEntry(entryWithInvalidRole);
            expect(report.isValid).toBe(false);
        });
    });
    describe("registry integrity", () => {
        it("initial catalog has unique source keys", () => {
            const keys = INITIAL_DISCOVERY_SOURCE_CATALOG.map((e) => e.key);
    ...
    describe("role and tier consistency", () => {
        ...
        it("filter by role returns correct subsets", () => {
            const authoritative = discoverySourceRegistry.getByRole(DiscoverySourceUsageRole.AUTHORITATIVE);
            expect(authoritative.length).toBe(4);
            const fallback = discoverySourceRegistry.getByRole(DiscoverySourceUsageRole.FALLBACK);
            expect(fallback.length).toBe(2);
        });
        it("filter by tier returns correct subsets", () => {
            const experimental = discoverySourceRegistry.getByTier(DiscoverySourceTier.EXPERIMENTAL);
            expect(experimental.length).toBe(1);
            const first = experimental[0];
            expect(first).toBeDefined();
            expect(first.key === "google-trends-it").toBe(true);
        });
...
//# sourceMappingURL=discovery-source-registry.test.js.map