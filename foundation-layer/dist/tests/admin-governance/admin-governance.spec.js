...
import { AuditLinkType, CompatibilityStatus, EnvironmentTier, FeatureFlagDefaultState, GovernanceDecisionStatus, GovernanceEnvironmentStatus, GovernanceModuleStatus, GovernanceSourceType, GuardrailSeverity, OverrideStatus, SafetyControlLevel, createActorRef, createAdminFeatureFlag, createAdminSafetyDecision, createAdminSafetyDecisionId, createEnvironmentControlState, createEnvironmentControlStateId, createEnvironmentKey, createFeatureFlag, createFeatureFlagId, createFeatureFlagKey, createGenerationGuardrail, createGenerationGuardrailId, createGovernanceAuditLink, createGovernanceAuditLinkId, createGovernanceCompatibilityView, createGovernanceCompatibilityViewId, createGovernanceAuditRef, createGovernanceMetadata, createGovernanceVersionTag, createModuleControl, createModuleControlId, createModuleKey, createOperationKey, createPolicyOverride, createPolicyOverrideId, createReason, createSourceEnablementControl, createSourceEnablementControlId, createSourceKey, createThreshold, validateAdminSafetyDecision, validateFeatureFlag, validateGenerationGuardrail, validateGovernanceAuditLink, validateGovernanceCompatibilityView, validateEnvironmentControlState, validateModuleControl, validatePolicyOverride, validateSourceEnablementControl, } from "../../src/admin-governance/index.js";
...
describe("admin-governance canonical naming + validators", () => {
    ...
    it("validates admin safety decision, audit link and compatibility view", () => {
        const decision = createAdminSafetyDecision({
            ...
            reasons: [createReason("risk-high")],
            metadata: createGovernanceMetadata(),
        });
        ...
        expect(validateAdminSafetyDecision({ ...decision, reasons: [createReason("risk"), createReason("risk")] }).isValid).toBe(false);
...
//# sourceMappingURL=admin-governance.spec.js.map