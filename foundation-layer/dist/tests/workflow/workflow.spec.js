...
import { applyTransition, canTransition, createInitialWorkflowTransition, getNextState, } from "@/workflow/foundation-workflow.machine.js";
...
describe("workflow machine", () => {
    it("matches transition map for all state/transition pairs", () => {
        ...
        const transitions = Object.values(WorkflowTransition).filter((transition) => transition !== WorkflowTransition.INITIALIZE);
    ...
    it("rejects illegal transition and non-monotonic timestamp", () => {
        const initialRecord = createInitialWorkflowTransition(createTimestamp("2026-01-01T00:00:00.000Z"));
        ...
        expect(() => applyTransition(instance, WorkflowTransition.FINALIZE, {
            at: createTimestamp("2026-01-02T00:00:00.000Z"),
        })).toThrow();
        expect(() => applyTransition(instance, WorkflowTransition.VALIDATE, {
    ...
    it("supports legal lifecycle with initialize/reject/reopen/archive policy", () => {
        ...
        expect(() => applyTransition(archived, WorkflowTransition.REOPEN, {
    ...
    it("detects illegal transition history through validator", () => {
        ...
        const report = validateWorkflowInstance(brokenHistory);
