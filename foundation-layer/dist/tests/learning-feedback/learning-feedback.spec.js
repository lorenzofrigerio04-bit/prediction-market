...
import { EDITORIAL_FEEDBACK_SIGNAL_SCHEMA_ID, EDITORIAL_FEEDBACK_SCHEMA_ID, FEEDBACK_AGGREGATION_SCHEMA_ID, FEEDBACK_SIGNAL_SCHEMA_ID, GENERATOR_IMPROVEMENT_ARTIFACT_SCHEMA_ID, IMPROVEMENT_ARTIFACT_SCHEMA_ID, LEARNING_AGGREGATION_SCHEMA_ID, LEARNING_COMPATIBILITY_RESULT_SCHEMA_ID, LEARNING_INSIGHT_SCHEMA_ID, LEARNING_RECOMMENDATION_SCHEMA_ID, OVERRIDE_PATTERN_SCHEMA_ID, RECOMMENDATION_SET_SCHEMA_ID, REJECTION_PATTERN_SCHEMA_ID, RELIABILITY_FEEDBACK_SCHEMA_ID, RELIABILITY_LEARNING_SIGNAL_SCHEMA_ID, AggregationStatus, DeterministicEditorialFeedbackAdapter, DeterministicReliabilityFeedbackAdapter, EditorialDecisionLearningAdapter, EditorialReviewLearningAdapter, FeedbackReasonCode, LearningCompatibilityStatus, FeedbackType, LearningFeedbackOverrideType, ImprovementArtifactType, LearningCompatibilityTarget, LearningInsightStatus, MarketDraftLearningAdapter, PatternStatus, PublishableCandidateLearningAdapter, RecommendationStatus, ReleaseGateLearningAdapter, ReleaseImpact, ReliabilityReportLearningAdapter, createEditorialFeedback, createOverridePattern, createRejectionPattern, createRejectionPatternId, createOverridePatternId, createCorrelationId, createEditorialFeedbackSignal, createEditorialFeedbackSignalId, createImprovementArtifact, createImprovementArtifactId, createLearningAggregation, createLearningAggregationId, createLearningInsight, createLearningInsightId, createLearningRecommendation, createLearningRecommendationId, createRecommendationSet, createFeedbackAggregation, createGeneratorImprovementArtifact, createReliabilityLearningSignal, createReliabilityLearningSignalId, createReliabilityFeedback, validateFeedbackSignal, validateEditorialFeedback, validateFeedbackAggregation, validateReliabilityFeedback, validateRecommendationSet, validateGeneratorImprovementArtifact, validateRejectionPattern, validateOverridePattern, validateEditorialFeedbackSignal, validateImprovementArtifact, validateLearningAggregation, validateLearningCompatibility, validateLearningInsight, validateLearningRecommendation, validateReliabilityLearningSignal, } from "@/learning-feedback/index.js";
const makeEditorialSignal = () => createEditorialFeedbackSignal({
...
const makeReliabilitySignal = () => createReliabilityLearningSignal({
...
const makeAggregation = () => createLearningAggregation({
...
const makeInsight = () => createLearningInsight({
...
const makeRecommendation = () => createLearningRecommendation({
...
const makeImprovement = () => createImprovementArtifact({
...
const makeRejectionPattern = () => createRejectionPattern({
    id: createRejectionPatternId("lrp_pattern001"),
    status: PatternStatus.ACTIVE,
    reason_codes: ["missing_evidence"],
    supporting_refs: ["evidence_1"],
});
const makeOverridePattern = () => createOverridePattern({
...
describe("learning-feedback module", () => {
    ...
    it("2) rejects editorial approval with blocking reason codes", () => {
        expect(() => createEditorialFeedbackSignal({
            ...makeEditorialSignal(),
    ...
    it("12) validates rejection and override patterns", () => {
        ...
        expect(validateRejectionPattern({ ...makeRejectionPattern(), supporting_refs: [] }).isValid).toBe(false);
        expect(validateOverridePattern({ ...makeOverridePattern(), supporting_refs: [] }).isValid).toBe(false);
    });
    ...
    it("17) keeps root and namespace exports stable", () => {
        ...
        const reliabilityViaAdapter = new DeterministicReliabilityFeedbackAdapter().adapt(makeReliabilitySignal());
...
//# sourceMappingURL=learning-feedback.spec.js.map