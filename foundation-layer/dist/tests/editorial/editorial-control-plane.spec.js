...
import { FinalReadinessStatus, PriorityLevel, QueueStatus, ReasonCode, ReviewStatus, ApprovalScope, ActionType, OverrideType, createApprovalDecision, createApprovalDecisionId, createApprovalScore, createAuditCorrelationId, createAuditRecord, createAuditRecordId, createAuditReferenceId, createBlockingFlag, createChangedFieldReference, createControlledStateTransition, createControlledStateTransitionId, createEditorialActorId, createEditorialReview, createEditorialReviewId, createGatingSummary, createManualOverride, createManualOverrideId, createPublicationReadyArtifact, createPublicationReadyArtifactId, createRequiredAction, createRejectionDecision, createRejectionDecisionId, createReviewQueueEntry, createReviewQueueEntryId, createRevisionRecord, createRevisionRecordId, createSeveritySummary, createOverrideScope, validateApprovalDecision, validateAuditRecord, validateControlledStateTransition, validateEditorialReview, validateManualOverride, validatePublicationReadyArtifact, validatePublishableCandidateEditorialCompatibility, validateReviewQueueEntry, validateRevisionRecord, DeterministicControlledStateTransitionManager, DeterministicPublicationReadinessEvaluator, DeterministicRevisionTracker, } from "@/index.js";
...
const makeQueueEntry = (candidateId, unresolvedBlocking = false) => createReviewQueueEntry({
    id: createReviewQueueEntryId("rqe_abcdefghi1"),
    version: createEntityVersion(1),
    publishable_candidate_id: candidateId,
...
const makeReview = (candidateId) => createEditorialReview({
...
describe("Editorial & Control Plane v1", () => {
    ...
    it("invalid revision record with empty changed fields", () => {
        ...
        const report = validateRevisionRecord(bad);
        expect(report.isValid).toBe(false);
    });
    ...
    it("deterministic controlled transition manager enforces rules", () => {
        ...
        expect(() => manager.validateTransition(transition, {
            queue_entry: queueEntry,
            approvals: [
                createApprovalDecision({
...
//# sourceMappingURL=editorial-control-plane.spec.js.map