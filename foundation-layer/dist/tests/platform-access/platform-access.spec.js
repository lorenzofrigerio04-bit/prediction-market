...
import { createActionPermissionCheckId, createAuthorizationDecisionId, createPermissionPolicyId, createRoleAssignmentId, createRoleDefinitionId, createUserIdentityId, createWorkspaceId, } from "../../src/platform-access/value-objects/platform-access-ids.vo.js";
...
const makeScope = () => createAccessScope({
...
const makeRole = () => createRoleDefinition({
    id: ROLE_ID,
...
const makeAssignment = () => createRoleAssignment({
...
const makeUser = (overrides) => createUserIdentity({
    id: USER_ID,
    version: createVersionTag("v1.0.0"),
...
const makePolicy = (overrides) => createPermissionPolicy({
...
const makeDecision = (overrides) => createAuthorizationDecision({
...
const makeEngine = (input) => new DeterministicAuthorizationEngine({
    ...
    capability_policy_adapter: new DeterministicCapabilityPolicyAdapter(new Map([[USER_ID, input?.grantedCapabilities ?? new Set()]])),
    platform_compatibility_adapter: new DeterministicPlatformCompatibilityAdapter(),
    access_scope_resolver: new DeterministicAccessScopeResolver(input?.resolverScopes ?? new Map([[USER_ID, [makeScope()]]])),
});
describe("platform-access v1 core requirements", () => {
    ...
    it("02 - validates active RoleAssignment with context user+role", () => {
        expect(validateRoleAssignment(makeAssignment(), undefined, {
            user_exists: () => true,
            role_exists: () => true,
        }).isValid).toBe(true);
    });
    ...
    it("05 - rejects DENIED AuthorizationDecision without blocking reasons", () => {
        ...
        expect(validateAuthorizationDecision(invalidDeniedDecision).isValid).toBe(false);
    });
    it("06 - rejects ALLOWED AuthorizationDecision with blocking reasons", () => {
        ...
        expect(validateAuthorizationDecision(invalidAllowedDecision).isValid).toBe(false);
    });
    it("07 - rejects PermissionPolicy when allowed_actions is empty", () => {
        ...
        expect(validatePermissionPolicy(invalidPolicy).isValid).toBe(false);
    });
    it("08 - rejects workspace-specific AccessScope without workspace_id", () => {
        ...
        expect(validateAccessScope(invalidScope).isValid).toBe(false);
    });
    it("08b - createAccessScope rejects workspace-module scope without workspace_id", () => {
        expect(() => createAccessScope({
            scope_type: ScopeType.WORKSPACE_MODULE,
            workspace_id_nullable: null,
            module_scope_nullable: TargetModule.EDITORIAL,
            entity_scope_nullable: null,
    ...
    it("09 - rejects ActionPermissionCheck PASSED when context decision is DENIED", () => {
        ...
        expect(validateActionPermissionCheck(createActionPermissionCheck({
        ...
        }), undefined, { decision: deniedDecision }).isValid).toBe(false);
    });
    it("10 - rejects ActionPermissionCheck when decision_ref does not match context decision", () => {
        ...
        }), undefined, { decision }).isValid).toBe(false);
    });
    ...
    it("14 - denies sensitive action when explicit capability grant is missing", () => {
        ...
        expect(decision.decision_status).toBe(DecisionStatus.DENIED);
        expect(decision.blocking_reasons.some((reason) => reason.includes(SENSITIVE_FLAG))).toBe(true);
    });
});
//# sourceMappingURL=platform-access.spec.js.map