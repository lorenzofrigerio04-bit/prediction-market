...
describe("validators", () => {
    it("rejects additional properties via schema validation", () => {
        ...
        const report = validateSourceRecord(input);
        expect(report.isValid).toBe(false);
        expect(report.issues.some((issue) => issue.code === "ADDITIONAL_PROPERTY_NOT_ALLOWED")).toBe(true);
    });
    it("returns deterministic code for binary market mismatch", () => {
        ...
        const market = createCandidateMarket({
            ...
            resolutionWindow: createResolutionWindow("2026-03-01T00:00:00.000Z", "2026-06-01T00:00:00.000Z"),
        ...
        expect(report.isValid).toBe(false);
        expect(report.issues.map((issue) => issue.code)).toContain("BINARY_MARKET_OUTCOME_MISMATCH");
    });
    it("produces deterministic generatedAt and stable issue ordering", () => {
        ...
        const report1 = validateSourceRecord(input, { generatedAt });
        const report2 = validateSourceRecord(input, { generatedAt });
        expect(report1.generatedAt).toBe(generatedAt);
        expect(report2.generatedAt).toBe(generatedAt);
        expect(report1.issues.map((issue) => `${issue.path}|${issue.code}|${issue.message}`)).toEqual(report2.issues.map((issue) => `${issue.path}|${issue.code}|${issue.message}`));
