...
import { CompositeOperationsConsoleCompatibilityAdapter, DeterministicActionSurfaceResolver, DeterministicArtifactInspectionBuilder, DeterministicAuditTimelineBuilder, DeterministicCandidateViewBuilder, DeterministicConsoleStateResolver, DeterministicPermissionAwareViewEvaluator, DeterministicQueueViewBuilder, DeterministicReadinessPanelBuilder, EditorialConsoleCompatibilityAdapter, OperationsConsoleActionKey, PlatformAccessConsoleCompatibilityAdapter, PublicationConsoleCompatibilityAdapter, QueueScope, ReliabilityConsoleCompatibilityAdapter, ReadinessStatus, VisibilityStatus, createActionConstraint, createActionConstraintRef, createActionSurface, createActionSurfaceId, createActorRef, createArtifactInspectionView, createArtifactInspectionViewId, createArtifactRef, createAuditTimelineItem, createAuditTimelineView, createAuditTimelineViewId, createBreadcrumbItem, createCandidateDetailView, createCandidateDetailViewId, createCandidateReadinessSnapshot, createCandidateRef, createConsoleFilterState, createConsoleNavigationState, createConsoleNavigationStateId, createDisplayLabel, createFilterToken, createOperationsConsoleCompatibilityResultId, createPermissionAwareViewState, createPermissionAwareViewStateId, createPermissionBasis, createPermissionEvaluationBasis, createQueueEntryRef, createQueueEntryView, createQueueFilter, createQueuePanelView, createQueuePanelViewId, createQueueSortConfig, createQueueSummaryCounts, createQueueVisibilityRule, createReadinessGatingItem, createReadinessPanelView, createReadinessPanelViewId, createReadinessRecommendedAction, createSortField, createSummaryCount, createWarningMessage, queuePanelViewSchema, queueEntryViewSchema, candidateListViewSchema, candidateDetailViewSchema, artifactInspectionViewSchema, auditTimelineItemSchema, auditTimelineViewSchema, readinessPanelViewSchema, actionSurfaceSchema, consoleNavigationStateSchema, permissionAwareViewStateSchema, validateActionSurface, validateArtifactInspectionView, validateAuditTimelineView, validateCandidateDetailView, validateConsoleNavigationState, validateOperationsConsoleCompatibility, validatePermissionAwareViewState, validateQueuePanelView, validateReadinessPanelView, } from "@/operations-console/index.js";
...
import { ACTION_SURFACE_SCHEMA_ID, ARTIFACT_INSPECTION_VIEW_SCHEMA_ID, AUDIT_TIMELINE_ITEM_SCHEMA_ID, AUDIT_TIMELINE_VIEW_SCHEMA_ID, CANDIDATE_DETAIL_VIEW_SCHEMA_ID, CANDIDATE_LIST_VIEW_SCHEMA_ID, CONSOLE_NAVIGATION_STATE_SCHEMA_ID, PERMISSION_AWARE_VIEW_STATE_SCHEMA_ID, QUEUE_ENTRY_VIEW_SCHEMA_ID, QUEUE_PANEL_VIEW_SCHEMA_ID, READINESS_PANEL_VIEW_SCHEMA_ID, SHARED_CONSOLE_SCHEMA_ID, } from "@/operations-console/schemas/index.js";
const makeQueueEntry = () => createQueueEntryView({
...
const makeQueuePanel = () => createQueuePanelView({
...
const makeCandidateDetail = () => createCandidateDetailView({
...
const makeArtifactInspection = () => createArtifactInspectionView({
...
const makeAuditTimeline = () => createAuditTimelineView({
    ...
    correlation_groups: [{ group_ref: "cgr_group001", item_refs: ["itm_001"] }],
    filter_state: { actor_refs: [], action_types: [], severity_levels: [] },
    visibility_status: VisibilityStatus.VISIBLE,
});
const makeReadiness = () => createReadinessPanelView({
...
const makeActionSurface = () => createActionSurface({
...
const makePermissionAwareState = () => createPermissionAwareViewState({
...
const makeNavigationState = () => createConsoleNavigationState({
...
describe("operations-console module", () => {
    it("1) valid QueuePanelView", () => {
        ...
        expect(new DeterministicQueueViewBuilder().buildQueueView({ view: makeQueuePanel() }).id).toBe(makeQueuePanel().id);
    });
    it("2) invalid QueuePanelView without panel_key", () => {
        const invalid = { ...makeQueuePanel(), panel_key: undefined };
        expect(validateQueuePanelView(invalid).isValid).toBe(false);
    });
    ...
    it("4) valid CandidateDetailView", () => {
        ...
        expect(report.isValid).toBe(true);
        expect(new DeterministicCandidateViewBuilder().buildCandidateDetailView({
            detail_view: makeCandidateDetail(),
            list_view: {
                id: "clv_candidatelist001",
                version: "v1.0.0",
        ...
        }).id).toBe(makeCandidateDetail().id);
    });
    it("5) invalid CandidateDetailView without valid target ref", () => {
        const invalid = { ...makeCandidateDetail(), candidate_ref: "invalid_ref" };
        expect(validateCandidateDetailView(invalid).isValid).toBe(false);
    });
    it("6) valid ArtifactInspectionView", () => {
        const report = validateArtifactInspectionView(makeArtifactInspection());
        expect(report.isValid).toBe(true);
        expect(new DeterministicArtifactInspectionBuilder().buildInspectionView({ view: makeArtifactInspection() }).id).toBe(makeArtifactInspection().id);
    });
    ...
    it("8) valid AuditTimelineView", () => {
        ...
        expect(report.isValid).toBe(true);
        expect(new DeterministicAuditTimelineBuilder().buildTimeline({ view: makeAuditTimeline() }).id).toBe(makeAuditTimeline().id);
    });
    it("9) invalid AuditTimelineView with missing timestamps", () => {
        ...
        expect(validateAuditTimelineView(invalid).isValid).toBe(false);
    });
    it("10) valid ReadinessPanelView", () => {
        const report = validateReadinessPanelView(makeReadiness());
        expect(report.isValid).toBe(true);
        expect(new DeterministicReadinessPanelBuilder().buildReadinessPanel({ view: makeReadiness() }).id).toBe(makeReadiness().id);
    });
    it("11) invalid ReadinessPanelView marked ready with blocking issues", () => {
        ...
        expect(validateReadinessPanelView(invalid).isValid).toBe(false);
    });
    it("12) valid ActionSurface", () => {
        const report = validateActionSurface(makeActionSurface());
        expect(report.isValid).toBe(true);
        expect(new DeterministicActionSurfaceResolver().resolve({ surface: makeActionSurface() }).id).toBe(makeActionSurface().id);
    });
    it("13) invalid ActionSurface exposing denied action", () => {
        ...
        expect(validateActionSurface(invalid).isValid).toBe(false);
    });
    ...
    it("15) valid PermissionAwareViewState", () => {
        ...
        expect(report.isValid).toBe(true);
        expect(new DeterministicPermissionAwareViewEvaluator().evaluate({ state: makePermissionAwareState() }).id).toBe(makePermissionAwareState().id);
    });
    it("16) invalid hidden PermissionAwareViewState with allowed actions", () => {
        ...
        expect(validatePermissionAwareViewState(invalid).isValid).toBe(false);
    });
    it("17) valid ConsoleNavigationState", () => {
        const report = validateConsoleNavigationState(makeNavigationState());
        expect(report.isValid).toBe(true);
        expect(new DeterministicConsoleStateResolver().resolve({ state: makeNavigationState() }).id).toBe(makeNavigationState().id);
    });
    it("18) invalid ConsoleNavigationState with inconsistent breadcrumb/panel state", () => {
        ...
        expect(validateConsoleNavigationState(invalid).isValid).toBe(false);
    });
    it("19) compatibility test with Platform Access artifacts", () => {
        ...
        expect(validateOperationsConsoleCompatibility(result, undefined, {
...
//# sourceMappingURL=operations-console.spec.js.map