...
const deriveStrategyType = (input) => {
    const relationCount = input.event_graph_context.reduce((total, node) => total + node.incoming_relations.length + node.outgoing_relations.length, 0);
    if (relationCount >= 8) {
        return StrategyType.BALANCED;
    }
    return StrategyType.CONSERVATIVE;
};
export class DeterministicExpansionStrategySelector {
    select(input) {
...
//# sourceMappingURL=deterministic-expansion-strategy-selector.js.map