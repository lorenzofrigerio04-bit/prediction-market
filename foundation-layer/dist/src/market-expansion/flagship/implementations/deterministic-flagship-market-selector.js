...
export class DeterministicFlagshipMarketSelector {
    select(input) {
        if (input.candidate_markets.length === 0) {
            throw new ValidationError("FLAGSHIP_SELECTION_FAILED", "candidate_markets must include at least one market");
        }
        const sorted = [...input.candidate_markets].sort((left, right) => right.confidenceScore - left.confidenceScore || left.id.localeCompare(right.id));
...
//# sourceMappingURL=deterministic-flagship-market-selector.js.map