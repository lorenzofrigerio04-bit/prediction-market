...
import { createDistinctMarketRefs, createFamilyKey, createMarketRef, createSourceContextRef, } from "../../value-objects/market-expansion-shared.vo.js";
export const getMarketFamilyTotalMarkets = (family) => 1 + family.satellite_market_refs.length + family.derivative_market_refs.length;
export const createMarketFamily = (input) => {
    ...
    const satellite_market_refs = createDistinctMarketRefs(input.satellite_market_refs, "satellite_market_refs");
    const derivative_market_refs = createDistinctMarketRefs(input.derivative_market_refs, "derivative_market_refs");
    if (satellite_market_refs.includes(flagship_market_ref)) {
        throw new ValidationError("INVALID_MARKET_FAMILY", "flagship_market_ref cannot appear in satellite_market_refs");
    }
    if (derivative_market_refs.includes(flagship_market_ref)) {
        throw new ValidationError("INVALID_MARKET_FAMILY", "flagship_market_ref cannot appear in derivative_market_refs");
    }
    const allRefs = [flagship_market_ref, ...satellite_market_refs, ...derivative_market_refs];
    if (new Set(allRefs).size !== allRefs.length) {
        throw new ValidationError("INVALID_MARKET_FAMILY", "all market references in family must be unique");
    }
    return deepFreeze({
        ...input,
        family_key,
...
//# sourceMappingURL=market-family.entity.js.map