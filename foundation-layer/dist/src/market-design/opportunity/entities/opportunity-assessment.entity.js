...
export const createOpportunityAssessment = (input) => {
    ...
    if (input.opportunity_status === OpportunityStatus.BLOCKED && blockingReasons.length === 0) {
        throw new ValidationError("INVALID_BLOCKING_CONSISTENCY", "blocking_reasons must be non-empty when opportunity_status is blocked");
    }
    if (input.opportunity_status === OpportunityStatus.ELIGIBLE && blockingReasons.length > 0) {
        throw new ValidationError("INVALID_BLOCKING_CONSISTENCY", "blocking_reasons must be empty when opportunity_status is eligible");
    }
    return deepFreeze({
        ...input,
        blocking_reasons: [...blockingReasons],
        recommendation_notes_nullable: input.recommendation_notes_nullable === null ? null : input.recommendation_notes_nullable.trim(),
    });
};
//# sourceMappingURL=opportunity-assessment.entity.js.map