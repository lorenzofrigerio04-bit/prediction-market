...
import { createContractSelection, } from "../../contract-selection/entities/contract-selection.entity.js";
import { createDeadlineResolution, } from "../../deadlines/entities/deadline-resolution.entity.js";
import { createOpportunityAssessment } from "../../opportunity/entities/opportunity-assessment.entity.js";
import { createOutcomeDefinition, } from "../../outcomes/entities/outcome-definition.entity.js";
...
const deriveOpportunityStatus = (blockingReasons, aggregateScore) => {
...
export class DeterministicOpportunityAssessor {
    assess(input) {
        const blockingReasons = input.blocking_reasons ?? [];
        const aggregateScore = (input.relevance_score +
...
export class DeterministicContractTypeSelector {
    select(input) {
...
const toOutcomeId = (suffix) => `out_${suffix.padEnd(6, "0").slice(0, 6)}`;
const buildBinaryOutcomes = (input) => {
...
const buildMultiOutcomes = (input) => {
    const outcomes = input.multi_input?.outcomes ?? [];
    return outcomes.map((outcome, index) => createOutcomeDefinition({
...
const buildScalarBracketOutcomes = (input) => {
...
export class DeterministicOutcomeGenerator {
    generate(input) {
        const outcomes = input.contract_type === ContractType.BINARY
...
export class DeterministicDeadlineResolver {
    resolve(input) {
...
export class DeterministicSourceHierarchySelector {
    select(input) {
...
const toMarketType = (contractType) => {
...
const toCandidateOutcomeType = (contractType) => {
    if (contractType === ContractType.BINARY) {
...
export class FoundationCompatibleMarketDraftBuilder {
    build(input) {
        const basis = input.foundation_target.resolution_basis ??
            MarketResolutionBasis.MULTI_SOURCE_CONSENSUS;
        const outcomes = input.outcome_generation_result.outcomes.map((outcome, index) => createMarketOutcome({
            id: createOutcomeId(`out_${(index + 1).toString().padStart(6, "0")}`),
            outcomeType: input.contract_selection.selected_contract_type === ContractType.BINARY
                ? index === 0
        ...
        return candidateMarketFactory({
            ...
            draftNotes: input.foundation_target.draft_notes_nullable ??
                `publish_score=${input.preliminary_scorecard.final_publish_score.toFixed(2)}`,
            entityVersion: createEntityVersion(input.foundation_target.entity_version),
        });
    }
}
//# sourceMappingURL=deterministic-core.builders.js.map