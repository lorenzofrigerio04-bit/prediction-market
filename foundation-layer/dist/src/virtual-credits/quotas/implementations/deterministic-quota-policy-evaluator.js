...
import { createNote, createQuotaEvaluationId, createVersion } from "../../value-objects/index.js";
const decide = (policy, current, requested) => {
    if (!policy.active) {
...
export class DeterministicQuotaPolicyEvaluator {
    evaluate(policy, currentUsage, requestedUsage, actionKey, targetAccountId) {
    ...
    evaluateMany(policies, context) {
        return policies.map((policy) => this.evaluate(policy, context.current_usage, context.requested_usage, context.action_key, context.target_account_id));
    }
}
//# sourceMappingURL=deterministic-quota-policy-evaluator.js.map