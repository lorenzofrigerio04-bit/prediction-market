...
export const validateSourceRecordInvariants = (sourceRecord) => {
    const issues = [];
    ...
    if (sourceRecord.publishedAt !== null) {
        ...
        if (Date.parse(sourceRecord.publishedAt) - Date.parse(sourceRecord.capturedAt) > maxFutureMs) {
            issues.push(errorIssue("PUBLISHED_AFTER_CAPTURED_EXCESSIVE", "/publishedAt", "publishedAt cannot exceed capturedAt by more than 24h"));
        }
    }
...
export const validateEventSignalInvariants = (eventSignal) => {
    const issues = [];
    if (eventSignal.sourceRecordIds.length < 1) {
        issues.push(errorIssue("MISSING_SOURCE_RECORD_IDS", "/sourceRecordIds", "At least one sourceRecordId is required"));
    }
    ...
    for (const duplicatedJurisdiction of duplicates(eventSignal.jurisdictions)) {
        issues.push(errorIssue("DUPLICATE_JURISDICTION", "/jurisdictions", "Duplicate jurisdiction", {
            duplicatedJurisdiction,
        }));
    }
    for (const duplicatedEntity of duplicates(eventSignal.involvedEntities)) {
        issues.push(errorIssue("DUPLICATE_INVOLVED_ENTITY", "/involvedEntities", "Duplicate involved entity", {
            duplicatedEntity,
        }));
    }
    return issues;
};
export const validateCanonicalEventInvariants = (canonicalEvent) => {
    const issues = [];
    if (Date.parse(canonicalEvent.firstObservedAt) > Date.parse(canonicalEvent.lastUpdatedAt)) {
        issues.push(errorIssue("LAST_UPDATED_BEFORE_FIRST_OBSERVED", "/lastUpdatedAt", "lastUpdatedAt must be >= firstObservedAt"));
    }
    ...
    for (const duplicatedSignalId of duplicates(canonicalEvent.supportingSignalIds)) {
        issues.push(errorIssue("DUPLICATE_SIGNAL_ID", "/supportingSignalIds", "Duplicate signal id", {
            duplicatedSignalId,
        }));
    }
    ...
    if ([EventStatus.RESOLVED, EventStatus.ARCHIVED].includes(canonicalEvent.status) &&
        canonicalEvent.resolutionWindow === null) {
        issues.push(errorIssue("MISSING_RESOLUTION_WINDOW", "/resolutionWindow", "resolutionWindow required for resolved/archived status"));
    }
    return issues;
};
export const validateStructuredClaimInvariants = (structuredClaim) => {
    const issues = [];
...
export const validateCandidateMarketInvariants = (candidateMarket) => {
    const issues = [];
    ...
    for (const [index, outcome] of candidateMarket.outcomes.entries()) {
        if (orderSet.has(outcome.orderIndex)) {
            issues.push(errorIssue("DUPLICATE_OUTCOME_ORDER_INDEX", `/outcomes/${index}/orderIndex`, "Duplicate order index"));
        }
    ...
    if (candidateMarket.marketType === "BINARY" &&
        (!outcomeTypes.has(CandidateOutcomeType.YES) || !outcomeTypes.has(CandidateOutcomeType.NO))) {
        issues.push(errorIssue("BINARY_MARKET_OUTCOME_MISMATCH", "/outcomes", "Binary markets require YES and NO outcomes"));
    }
    return issues;
};
export const validateMarketOutcomeInvariants = (marketOutcome) => {
    const issues = [];
...
export const validateWorkflowInstanceInvariants = (workflowInstance) => {
    const issues = [];
    ...
    if (initial.from !== null ||
        initial.to !== WorkflowState.DRAFT ||
        initial.transition !== WorkflowTransition.INITIALIZE) {
        issues.push(errorIssue("INVALID_INITIAL_TRANSITION", "/transitionHistory/0", "Initial transition must be null -> DRAFT via INITIALIZE"));
    }
    if (last.to !== workflowInstance.currentState) {
        issues.push(errorIssue("WORKFLOW_STATE_HISTORY_MISMATCH", "/currentState", "currentState must match last transition 'to' state"));
    }
    if (last.from !== workflowInstance.previousState) {
        issues.push(errorIssue("WORKFLOW_STATE_HISTORY_MISMATCH", "/previousState", "previousState must match last transition 'from' state"));
    }
    if (last.at !== workflowInstance.lastTransitionAt) {
        issues.push(errorIssue("WORKFLOW_LAST_TRANSITION_MISMATCH", "/lastTransitionAt", "lastTransitionAt must match last transition timestamp"));
    }
    for (let index = 1; index < history.length; index += 1) {
        const previous = history[index - 1];
        const current = history[index];
        if (current.from !== previous.to) {
            issues.push(errorIssue("INVALID_TRANSITION_SEQUENCE", `/transitionHistory/${index}`, "Transition history must be state-continuous"));
        }
        if (current.from === null) {
            issues.push(errorIssue("INVALID_INITIAL_TRANSITION", `/transitionHistory/${index}`, "Only first transition can start from null"));
        }
        else {
            const nextState = getNextState(current.from, current.transition);
            if (nextState !== current.to) {
                issues.push(errorIssue("INVALID_STATE_TRANSITION", `/transitionHistory/${index}`, "Transition not allowed by workflow rules", {
                    from: current.from,
        ...
        if (Date.parse(previous.at) > Date.parse(current.at)) {
            issues.push(errorIssue("WORKFLOW_HISTORY_NOT_MONOTONIC", `/transitionHistory/${index}`, "Transition history timestamps must be monotonic"));
        }
    }
    return issues;
};
//# sourceMappingURL=domain-invariants.validator.js.map