...
const validateCandidateDetailInvariants = (input) => {
    const issues = [];
    ...
    if (input.visibility_status === VisibilityStatus.HIDDEN && input.visible_actions.length > 0) {
        issues.push(errorIssue("HIDDEN_CANDIDATE_VIEW_ACTIONS_NOT_ALLOWED", "/visible_actions", "hidden candidate detail cannot expose visible_actions"));
    }
    if (input.visibility_status === VisibilityStatus.VISIBLE && input.visible_actions.includes(ActionKey.OPEN_DETAIL) === false) {
        issues.push(errorIssue("VISIBLE_CANDIDATE_VIEW_REQUIRES_OPEN_DETAIL", "/visible_actions", "visible candidate detail should include open_detail action"));
    }
    return issues;
};
export const validateCandidateDetailView = (input, options) => {
    ...
    return buildValidationReport("CandidateDetailView", input.id, [...schemaIssues, ...invariantIssues], resolveGeneratedAt(options));
};
//# sourceMappingURL=validate-candidate-detail-view.js.map