...
const mergeIssues = (...issues) => issues.flat();
export const validateLiveIntegrationPipeline = (input) => {
    ...
    if (input.publication_handoff.publication_package_id !== input.publication_package.id) {
        crossIssues.push(errorIssue("HANDOFF_PACKAGE_MISMATCH", "/publication_handoff/publication_package_id", "publication handoff must reference publication package id"));
    }
    if (input.scheduling_candidate.publication_package_id !== input.publication_package.id) {
        crossIssues.push(errorIssue("SCHEDULING_PACKAGE_MISMATCH", "/scheduling_candidate/publication_package_id", "scheduling candidate must reference publication package id"));
    }
    if (input.live_publication_contract.publication_package_id !== input.publication_package.id) {
        crossIssues.push(errorIssue("CONTRACT_PACKAGE_MISMATCH", "/live_publication_contract/publication_package_id", "live publication contract must reference publication package id"));
    }
    if (input.delivery_readiness_report.publication_package_id !== input.publication_package.id) {
        crossIssues.push(errorIssue("READINESS_PACKAGE_MISMATCH", "/delivery_readiness_report/publication_package_id", "delivery readiness report must reference publication package id"));
    }
    if ((input.delivery_readiness_report.readiness_status === ReadinessStatus.BLOCKED ||
        ...
        crossIssues.push(errorIssue("READY_SCHEDULING_BLOCKED_BY_READINESS", "/scheduling_candidate/scheduling_status", "scheduling candidate cannot be READY when delivery readiness is BLOCKED or FAILED"));
    }
    if (input.publication_package.package_status === PackageStatus.INVALID &&
        ...
        crossIssues.push(errorIssue("READY_CONTRACT_FROM_INVALID_PACKAGE", "/live_publication_contract/contract_status", "READY live publication contract cannot be built from INVALID package"));
    }
    const issues = mergeIssues(packageReport.issues, handoffReport.issues, schedulingReport.issues, contractReport.issues, readinessReport.issues, crossIssues);
...
//# sourceMappingURL=validate-live-integration-pipeline.js.map