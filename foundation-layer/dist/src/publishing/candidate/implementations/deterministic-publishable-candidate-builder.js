...
import { createMarketDraftPipelineId, createPublishableCandidateId, } from "../../value-objects/publishing-ids.vo.js";
...
import { createPublishableCandidate, } from "../entities/publishable-candidate.entity.js";
const toPipelineId = (marketId) => createMarketDraftPipelineId(`mdp_${marketId.slice(4)}pl`);
export class DeterministicPublishableCandidateBuilder {
    build(input) {
        ...
        const sectionTypes = new Set(input.rulebook_compilation.included_sections.map((section) => section.section_type));
        ...
        return createPublishableCandidate({
            id: createPublishableCandidateId(`pcnd_${createDeterministicToken(`${pipelineId}|${input.title_set.id}|${input.rulebook_compilation.id}`)}can`),
...
//# sourceMappingURL=deterministic-publishable-candidate-builder.js.map