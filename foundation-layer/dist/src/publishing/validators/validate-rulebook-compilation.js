...
const validateRulebookCompilationInvariants = (input) => {
    const issues = [];
    ...
    for (const requiredType of REQUIRED_RULEBOOK_SECTION_TYPES) {
        if (!sectionTypes.includes(requiredType)) {
            issues.push(errorIssue("MISSING_REQUIRED_RULEBOOK_SECTION", "/included_sections", `Missing required section type: ${requiredType}`));
        }
    }
    ...
    if (new Set(input.included_sections.map((section) => section.section_type)).size !== input.included_sections.length) {
        issues.push(errorIssue("DUPLICATE_RULEBOOK_SECTION_TYPE", "/included_sections", "included_sections must not contain duplicate section types", { dedupKey }));
    }
    return issues;
};
export const validateRulebookCompilation = (input, options) => {
...
//# sourceMappingURL=validate-rulebook-compilation.js.map