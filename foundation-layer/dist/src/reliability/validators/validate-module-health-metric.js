...
const expectedStatusFromThreshold = (input) => {
...
const validateModuleHealthMetricInvariants = (input) => {
    const issues = [];
    const expectedStatus = expectedStatusFromThreshold(input);
    if (expectedStatus !== null && input.threshold_status !== expectedStatus) {
        issues.push(errorIssue("THRESHOLD_STATUS_MISMATCH", "/threshold_status", "ModuleHealthMetric.thresholdStatus must be coherent with metricValue and threshold metadata"));
    }
    return issues;
};
export const validateModuleHealthMetric = (input, options) => {
    ...
    return buildValidationReport("ModuleHealthMetric", input.id, [...schemaIssues, ...invariantIssues], resolveGeneratedAt(options));
};
//# sourceMappingURL=validate-module-health-metric.js.map