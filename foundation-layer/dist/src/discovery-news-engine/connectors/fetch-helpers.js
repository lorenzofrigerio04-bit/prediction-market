 ...
 */
export async function executeFetch(request, options) {
 ...
 */
export function inferContentType(contentType) {
    const lower = contentType.toLowerCase();
    if (lower.includes("application/json"))
        return DiscoveryContentType.APPLICATION_JSON;
    if (lower.includes("application/xml") || lower.includes("text/xml"))
        return DiscoveryContentType.APPLICATION_XML;
    ...
    if (lower.includes("text/html"))
        return DiscoveryContentType.TEXT_HTML;
    if (lower.includes("text/plain"))
        return DiscoveryContentType.TEXT_PLAIN;
    return null;
}
