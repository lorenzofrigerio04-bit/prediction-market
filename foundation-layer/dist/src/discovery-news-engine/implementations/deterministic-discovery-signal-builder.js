...
function slugForSignalId(sourceId, externalItemId) {
...
function timeWindowFromPublishedAt(publishedAt) {
...
function buildSignalFromItem(item, sourceId, provenanceMetadata, options) {
 ...
 */
export const deterministicDiscoverySignalBuilder = {
    build(input, options) {
        const items = Array.isArray(input.items)
            ? input.items
            : [input];
        const payload = input;
        const sourceId = "sourceId" in payload && payload.sourceId
            ? payload.sourceId
            : items[0].sourceReference.sourceId;
        const provenanceMetadata = "provenanceMetadata" in payload && payload.provenanceMetadata
        ...
        return items.map((item) => buildSignalFromItem(item, sourceId, provenanceMetadata, options));
    },
};
//# sourceMappingURL=deterministic-discovery-signal-builder.js.map