...
function toDayBucket(isoTimestamp) {
 ...
 */
function findClusterWithMember(clusters, getItem, predicate) {
 ...
 */
export function evaluateStoryClusterRules(candidate, candidateItemId, existingClusters, getItemForNormalizedId) {
    const getItem = getItemForNormalizedId;
    // 1. Canonical URL continuity
    const urlNorm = candidate.canonicalUrl?.trim();
    if (urlNorm) {
        const found = findClusterWithMember(existingClusters, getItem, (member) => (member.canonicalUrl?.trim() ?? "") === urlNorm);
        if (found) {
            return {
                match: true,
    ...
    if (sourceKey && candidate.externalItemId?.trim()) {
        ...
        const found = findClusterWithMember(existingClusters, getItem, (member) => getSourceKey(member) === sourceKey && (member.externalItemId?.trim() ?? "") === extId);
        if (found) {
            return {
                match: true,
                clusterId: found.clusterId,
    ...
    if (sourceKey && locator && STRUCTURED_SOURCE_KEYS_FOR_LOCATOR.has(sourceKey)) {
        const found = findClusterWithMember(existingClusters, getItem, (member) => getSourceKey(member) === sourceKey &&
    ...
    if (candidateHeadlineNorm && candidateDay) {
        const found = findClusterWithMember(existingClusters, getItem, (member) => normalizeHeadline(member.headline ?? "") === candidateHeadlineNorm &&
...
//# sourceMappingURL=discovery-story-clustering-rules.js.map