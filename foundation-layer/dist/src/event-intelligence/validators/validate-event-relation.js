...
const validateEventRelationInvariants = (input) => {
    const issues = [];
    if (input.source_event_id === input.target_event_id &&
        !SELF_RELATION_ALLOWED_TYPES.has(input.relation_type)) {
        issues.push(errorIssue("INVALID_SELF_RELATION", "/target_event_id", "source_event_id and target_event_id cannot be equal for this relation_type"));
    }
    return issues;
};
export const validateEventRelation = (input, options) => {
...
//# sourceMappingURL=validate-event-relation.js.map