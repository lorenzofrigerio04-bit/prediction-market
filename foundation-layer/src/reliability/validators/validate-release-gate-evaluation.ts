...
import type { ReleaseGateEvaluation } from "../release-gates/entities/release-gate-evaluation.entity.js";
import { RELEASE_GATE_EVALUATION_SCHEMA_ID } from "../schemas/release-gate-evaluation.schema.js";

const validateReleaseGateEvaluationInvariants = (input: ReleaseGateEvaluation): readonly ValidationIssue[] => {
  const issues: ValidationIssue[] = [];
  const hasMandatoryGateFailure =
  ...
  if (input.final_gate_status === FinalGateStatus.PASSED && hasMandatoryGateFailure) {
    issues.push(
      errorIssue(
        "INVALID_FINAL_GATE_STATUS",
        "/final_gate_status",
        "ReleaseGateEvaluation.finalGateStatus cannot be PASSED when mandatory gates contain false",
      ),
    );
  }
  return issues;
...
export const validateReleaseGateEvaluation = (
  input: ReleaseGateEvaluation,
