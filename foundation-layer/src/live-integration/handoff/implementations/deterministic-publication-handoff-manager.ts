...
import { createPublicationHandoff, type PublicationHandoff } from "../entities/publication-handoff.entity.js";
...
export class DeterministicPublicationHandoffManager implements PublicationHandoffManager {
  createHandoff(input: CreateHandoffInput): PublicationHandoff {
    const token = createDeterministicToken(
      `${input.publication_package_id}|${input.initiated_by}|${input.initiated_at}|${input.audit_ref}`,
    );
  ...
  validateAuditLinkage(input: PublicationHandoff) {
