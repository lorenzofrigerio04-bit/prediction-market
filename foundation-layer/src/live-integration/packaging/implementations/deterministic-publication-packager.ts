...
import { createPublicationPackage, type PublicationPackage } from "../entities/publication-package.entity.js";
...
const toPackageStatus = (status: FinalReadinessStatus): PackageStatus =>
  status === FinalReadinessStatus.APPROVED ? PackageStatus.VALIDATED : PackageStatus.INVALID;

export class DeterministicPublicationPackager implements PublicationPackager {
  createPackage(input: CreatePackageInput): PublicationPackage {
    const token = createDeterministicToken(
      `${input.publication_ready_artifact.id}|${input.version}|${input.created_at}`,
    );
  ...
  validatePackage(input: PublicationPackage) {
