...
const validateDeliveryReadinessReportInvariants = (
  input: DeliveryReadinessReport,
  ...
  if (input.readiness_status === ReadinessStatus.READY && input.blocking_issues.length > 0) {
    issues.push(
      errorIssue(
        "READY_CANNOT_HAVE_BLOCKING_ISSUES",
        "/blocking_issues",
...
export const validateDeliveryReadinessReport = (
  input: DeliveryReadinessReport,
