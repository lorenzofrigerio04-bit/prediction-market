...
const validateInvariants = (input: MarketFamily): readonly ValidationIssue[] => {
  const issues: ValidationIssue[] = [];
  if (input.flagship_market_ref.trim().length === 0) {
    issues.push(
      errorIssue("FAMILY_FLAGSHIP_REQUIRED", "/flagship_market_ref", "flagship_market_ref is required"),
    );
  }
  if (new Set(input.satellite_market_refs).size !== input.satellite_market_refs.length) {
    issues.push(
      errorIssue(
        "FAMILY_SATELLITE_DUPLICATE_REFS",
        "/satellite_market_refs",
        "satellite_market_refs must be unique",
      ),
    );
  }
  if (new Set(input.derivative_market_refs).size !== input.derivative_market_refs.length) {
    issues.push(
      errorIssue(
        "FAMILY_DERIVATIVE_DUPLICATE_REFS",
        "/derivative_market_refs",
        "derivative_market_refs must be unique",
      ),
    );
  }
  const allRefs = [
...
export const validateMarketFamily = (input: MarketFamily, options?: ValidationOptions): ValidationReport => {
