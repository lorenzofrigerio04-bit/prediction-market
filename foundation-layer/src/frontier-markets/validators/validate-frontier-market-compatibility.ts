...
import type { AdvancedMarketCompatibilityResult } from "../compatibility/entities/advanced-market-compatibility-result.entity.js";
...
const validateCompatibilityInvariants = (
  ...
  if (Object.keys(input.mapped_artifact).length === 0) {
    issues.push(
      errorIssue("COMPATIBILITY_ARTIFACT_EMPTY", "/mapped_artifact", "mapped_artifact must not be empty"),
    );
  }
  ...
  if (typeof readiness !== "string") {
    ...
    issues.push(
      errorIssue(
        "COMPATIBILITY_READINESS_STATUS_MISMATCH",
        "/mapped_artifact/readiness",
  ...
  if (
    validationStatus === AdvancedValidationStatus.INVALID &&
    input.status === AdvancedCompatibilityStatus.COMPATIBLE
  ) {
    issues.push(
      errorIssue(
        "COMPATIBILITY_PROMOTION_BLOCKED_BY_VALIDATION_STATUS",
        "/status",
        "compatibility cannot be compatible when validation_status is invalid",
      ),
    );
  }
  return issues;
...
export const validateFrontierMarketCompatibility = (
  input: AdvancedMarketCompatibilityResult,
