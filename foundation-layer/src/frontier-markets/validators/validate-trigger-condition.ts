...
import type { TriggerCondition } from "../conditional/entities/trigger-condition.entity.js";

const validateTriggerInvariants = (input: TriggerCondition): readonly ValidationIssue[] => {
  const issues: ValidationIssue[] = [];
  if (input.trigger_type === TriggerType.UPSTREAM_EVENT && input.upstream_event_ref_or_market_ref.kind !== "upstream_event") {
    issues.push(
      errorIssue(
        "TRIGGER_TYPE_REFERENCE_MISMATCH",
        "/upstream_event_ref_or_market_ref",
        "trigger_type upstream_event requires upstream_event reference",
      ),
    );
  }
  if (
    input.trigger_type === TriggerType.UPSTREAM_MARKET &&
    ...
    issues.push(
      errorIssue(
        "TRIGGER_TYPE_REFERENCE_MISMATCH",
        "/upstream_event_ref_or_market_ref",
        "trigger_type upstream_market requires upstream_market reference",
      ),
    );
  }
  return issues;
...
export const validateTriggerCondition = (
