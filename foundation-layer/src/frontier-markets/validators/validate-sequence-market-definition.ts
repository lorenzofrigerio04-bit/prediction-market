...
const validateSequenceInvariants = (input: SequenceMarketDefinition): readonly ValidationIssue[] => {
  const issues: ValidationIssue[] = [];
  if (input.sequence_targets.length < 2) {
    issues.push(
      errorIssue("SEQUENCE_TARGETS_INSUFFICIENT", "/sequence_targets", "sequence must have at least two targets"),
    );
  }
  ...
  if (requiredTargets.length === 0 && input.completion_policy === CompletionPolicy.ALL_REQUIRED) {
    issues.push(
      errorIssue(
        "SEQUENCE_REQUIRED_TARGETS_MISSING",
        "/sequence_targets",
        "all_required completion policy requires at least one required target",
      ),
    );
  }
  if (
    input.required_order_policy === RequiredOrderPolicy.STRICT &&
    input.sequence_targets.some((target) => target.canonical_event_ref_or_predicate.kind === "deterministic_predicate")
  ) {
    issues.push(
      errorIssue(
        "SEQUENCE_STRICT_ORDER_WEAK_REFERENCE",
        "/required_order_policy",
        "strict required order with predicate-only targets should be reviewed",
      ),
    );
  }
  if (requiredTargets.length === 0 && input.sequence_validation_status === SequenceValidationStatus.VALID) {
    issues.push(
      errorIssue(
        "SEQUENCE_STATUS_INCONSISTENT",
        "/sequence_validation_status",
        "sequence cannot be valid when required targets are missing",
      ),
    );
  }
  return issues;
...
export const validateSequenceMarketDefinition = (
  input: SequenceMarketDefinition,
