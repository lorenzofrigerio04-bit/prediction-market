...
const derivePublishingCompatibility = (
  contract: FrontierContract,
): Readonly<{ contract_type: ContractType; status: AdvancedCompatibilityStatus; notes: readonly string[] }> => {
  if ("race_targets" in contract) {
    ...
    return {
      ...
      notes:
        contract.race_validation_status === RaceValidationStatus.REVIEW_REQUIRED
  ...
  if ("sequence_targets" in contract) {
    ...
    return {
      ...
      status: AdvancedCompatibilityStatus.COMPATIBLE_WITH_WARNINGS,
      notes:
        contract.sequence_validation_status === SequenceValidationStatus.REVIEW_REQUIRED
...
const applyValidationStatusGate = (
  compatibility: Readonly<{
    contract_type: ContractType;
    status: AdvancedCompatibilityStatus;
    notes: readonly string[];
  }>,
  validationReport?: AdvancedContractValidationReport,
) => {
  if (
    ...
    return {
      ...compatibility,
      status:
        compatibility.status === AdvancedCompatibilityStatus.INCOMPATIBLE
...
export class PublishingEngineCompatibilityAdapter implements AdvancedContractCompatibilityAdapter {
  adapt(contract: FrontierContract, validation_report?: AdvancedContractValidationReport) {
    const compatibility = applyValidationStatusGate(
      derivePublishingCompatibility(contract),
      validation_report,
    );
