...
const toEditorialContractType = (contract: FrontierContract): ContractType => {
...
const toNeedsManualReview = (contract: FrontierContract): boolean => {
  if ("race_targets" in contract) {
...
const applyValidationStatusGate = (
  ...
  return { status, needsManualReview } as const;
};

export class EditorialPipelineCompatibilityAdapter implements AdvancedContractCompatibilityAdapter {
  adapt(contract: FrontierContract, validation_report?: AdvancedContractValidationReport) {
    ...
    const gated = applyValidationStatusGate(
      needsManualReview
        ? AdvancedCompatibilityStatus.COMPATIBLE_WITH_WARNINGS
        : AdvancedCompatibilityStatus.COMPATIBLE,
      needsManualReview,
      validation_report,
    );
    return createAdvancedMarketCompatibilityResult({
      ...
      notes: [
        ...
        ...(gated.needsManualReview
          ? [
              createCompatibilityNote(
                "contract status requires manual editorial review before promotion",
              ),
            ]
          : []),
        ...
        validation_report.validation_status !== AdvancedValidationStatus.VALID
          ? [
              createCompatibilityNote(
                "advanced validation status is not ready; editorial flow remains manual-review gated",
              ),
            ]
          : []),
      ],
