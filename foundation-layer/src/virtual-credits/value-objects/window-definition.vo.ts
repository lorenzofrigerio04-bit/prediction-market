...
export const createWindowDefinition = (value: WindowDefinition): WindowDefinitionVo => {
  ...
  if (value.unit === MeasurementWindowUnit.LIFETIME && value.size !== 1) {
    throw new ValidationError(
      "INVALID_LIFETIME_WINDOW_SIZE",
      "window_definition.size must be 1 for lifetime windows",
      { size: value.size },
    );
  }
  return deepFreeze({ ...value }) as WindowDefinitionVo;
};
