...
import type { LearningCompatibilityResult } from "../compatibility/entities/learning-compatibility-result.entity.js";
...
const validateCompatibilityInvariants = (
  ...
  if (input.correlation_id.trim().length === 0) {
    issues.push(
      errorIssue("COMPATIBILITY_CORRELATION_ID_REQUIRED", "/correlation_id", "correlation_id is required"),
    );
  }
  if (Object.keys(input.mapped_artifact).length === 0) {
    issues.push(
      errorIssue(
        "COMPATIBILITY_MAPPED_ARTIFACT_EMPTY",
        "/mapped_artifact",
  ...
  if (typeof sourceId !== "string" || sourceId.trim().length === 0) {
    issues.push(
      errorIssue(
        "COMPATIBILITY_SOURCE_ID_MISSING",
        "/mapped_artifact/source_id",
        "mapped_artifact.source_id is required",
      ),
    );
  }
  ...
  if (typeof targetId !== "string" || targetId.trim().length === 0) {
    issues.push(
      errorIssue(
        "COMPATIBILITY_TARGET_ID_MISSING",
        "/mapped_artifact/target_id",
        "mapped_artifact.target_id is required",
      ),
    );
  }
  ...
  if (typeof readiness !== "string") {
    issues.push(
      errorIssue(
        "COMPATIBILITY_READINESS_MISSING",
    ...
    issues.push(
      errorIssue(
        "COMPATIBILITY_READINESS_MISMATCH",
        "/mapped_artifact/readiness",
  ...
  if (!Array.isArray(lossy)) {
    issues.push(
      errorIssue(
        "COMPATIBILITY_LOSSY_FIELDS_MISSING",
        "/mapped_artifact/lossy_fields",
        "mapped_artifact.lossy_fields must be an array",
      ),
    );
  }
  if (
    input.status === LearningCompatibilityStatus.COMPATIBLE &&
    input.notes.some((note) => note.toLowerCase().includes("incompatible"))
  ) {
    issues.push(
      errorIssue(
        "COMPATIBILITY_STATUS_NOTE_MISMATCH",
        "/notes",
        "compatible status cannot contain incompatible notes",
      ),
    );
  }
  return issues;
...
export const validateLearningCompatibility = (
  input: LearningCompatibilityResult,
