...
const validateEditorialReviewInvariants = (input: EditorialReview): readonly ValidationIssue[] => {
  ...
  if (summary.total_findings !== input.severity_summary.total_findings) {
    issues.push(
      errorIssue(
        "SEVERITY_SUMMARY_MISMATCH",
        "/severity_summary/total_findings",
        "severity_summary.total_findings must match findings",
      ),
    );
  }
  if (summary.highest_severity !== input.severity_summary.highest_severity) {
    issues.push(
      errorIssue(
        ...
        "severity_summary.highest_severity must match findings severities",
      ),
    );
  }
  return issues;
...
export const validateEditorialReview = (
