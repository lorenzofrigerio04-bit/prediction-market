...
export const createGatingSummary = (input: GatingSummary): GatingSummary => {
  ...
  if (!Number.isInteger(input.unresolved_blocking_flags_count) || input.unresolved_blocking_flags_count < 0) {
    throw new ValidationError(
      "INVALID_GATING_SUMMARY",
      "unresolved_blocking_flags_count must be a non-negative integer",
    );
  }
