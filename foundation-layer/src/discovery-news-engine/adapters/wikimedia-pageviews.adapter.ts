...
const WIKIMEDIA_ITEM_KEYS = ["items"] as const;
...
function buildWikiUrl(project: string, article: string): string {
...
function projectToLanguage(project: string): string {
  const match = project.match(/^([a-z]{2,3})\./);
  return match ? match[1]! : "en";
}

export const wikimediaPageviewsNormalizationAdapter: DiscoveryNormalizationAdapter = {
  normalize(payload: DiscoveryFetchedPayload, context: DiscoveryNormalizationContext): NormalizedDiscoveryPayload {
    ...
    for (const rec of itemsArray) {
      ...
      const observedMetricsNullable =
        views != null ? createDiscoveryObservedMetrics({ pageviewsNullable: views }) : null;
...
export const wikimediaPageviewsSourceAdapter: DiscoverySourceAdapter = {
