...
const DISCOVERY_SOURCE_ADAPTERS: readonly DiscoverySourceAdapter[] = [
...
const adapterByKey = new Map<string, DiscoverySourceAdapter>(
  DISCOVERY_SOURCE_ADAPTERS.map((a) => [String(a.sourceKey), a]),
);

function dummyDefinition(kind: DiscoverySourceKind) {
...
} from "./wikimedia-pageviews.adapter.js";
export {
  youtubeDataItSourceAdapter,
  youtubeDataItNormalizationAdapter,
} from "./youtube-data-it.adapter.js";
export {
  googleTrendsItSourceAdapter,
  googleTrendsItNormalizationAdapter,
} from "./google-trends-it.adapter.js";

export function getAdapterByKey(key: DiscoverySourceKey | string): DiscoverySourceAdapter | undefined {
  return adapterByKey.get(String(key));
}

export function getSupportedSourceKeys(): string[] {
  return DISCOVERY_SOURCE_ADAPTERS.map((a) => String(a.sourceKey));
}

export function getAdaptersByKind(kind: DiscoverySourceKind): DiscoverySourceAdapter[] {
