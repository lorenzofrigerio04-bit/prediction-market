...
function getItemTimestampMs(item: NormalizedDiscoveryItem): number {
  return Date.parse(item.publishedAt);
}

function getSignalTimestampMs(signal: DiscoverySignal): number {
 ...
 */
export function computeDiscoveryTrendIndicators(
  input: DiscoveryTrendIndicatorInput,
): DiscoveryTrendIndicatorSet {
  const {
    cluster,
    summary,
    itemsById,
  ...
  const inWindowItems: NormalizedDiscoveryItem[] = [];
  const inWindowSignals: DiscoverySignal[] = [];

  for (const id of itemIds) {
    const item = itemsById.get(id);
    if (!item) continue;
    const ms = getItemTimestampMs(item);
    if (ms >= windowStartMs && ms <= windowEndMs) inWindowItems.push(item);
  }
  for (const id of signalIds) {
    const sig = signalsById.get(id);
    if (!sig) continue;
    const ms = getSignalTimestampMs(sig);
    if (ms >= windowStartMs && ms <= windowEndMs) inWindowSignals.push(sig);
  }
  ...
  for (const item of inWindowItems) allTimestamps.push(getItemTimestampMs(item));
  for (const sig of inWindowSignals) allTimestamps.push(getSignalTimestampMs(sig));
  const timeSpanHours =
  ...
  let freshnessClassConsistency: DiscoveryTrendIndicatorSet["freshnessClassConsistency"] = "mixed";
  if (freshnessClasses.length > 0) {
    const uniq = new Set(freshnessClasses);
    if (uniq.size === 1) {
      const single = freshnessClasses[0]!;
      if (single === DiscoverySignalFreshnessClass.REALTIME) freshnessClassConsistency = "realtime";
