...
import type { EventLeadExtractionEvaluator } from "../interfaces/event-lead-extraction-evaluator.js";
import type {
  EventLeadExtractionContext,
  EventLeadExtractionResult,
} from "../interfaces/event-lead-extraction-evaluator.js";
...
import type { EventLeadEvidenceSet } from "../entities/event-lead-evidence-set.entity.js";
import { createEventLeadEvidenceSet } from "../entities/event-lead-evidence-set.entity.js";
import type { EventLeadReason } from "../entities/event-lead-reason.entity.js";
import { createEventLeadReason } from "../entities/event-lead-reason.entity.js";
import {
  createEventLeadExtractionDecisionLead,
  createEventLeadExtractionDecisionNotExtracted,
} from "../entities/event-lead-extraction-decision.entity.js";
...
function buildEventLeadId(clusterId: string, index: number): string {
...
function buildEvidenceSet(
  entry: DiscoveryRankingEntry,
  summariesByClusterId: ReadonlyMap<string, DiscoveryStoryClusterSummary> | undefined,
...
function leadReasonsFromEntry(entry: DiscoveryRankingEntry): EventLeadReason[] {
  const reasons: EventLeadReason[] = [];
...
function deriveConfidence(entry: DiscoveryRankingEntry): EventLeadConfidence {
  ...
  if (positiveCount >= 5 && entry.priorityClass === DiscoveryPriorityClass.HIGH) return EventLeadConfidence.HIGH;
  if (positiveCount >= 3) return EventLeadConfidence.MEDIUM;
  return EventLeadConfidence.LOW;
}

function italianContext(entry: DiscoveryRankingEntry): string | null {
  if (entry.breakdown.italianRelevance === "none") return null;
  return `italian_relevance=${entry.breakdown.italianRelevance}`;
}

function sourceSupportContext(entry: DiscoveryRankingEntry): string | null {
  const parts: string[] = [];
  if (entry.breakdown.authoritativeRelevance) parts.push("authoritative");
  if (entry.breakdown.editorialRelevance) parts.push("editorial");
  if (entry.breakdown.attentionRelevance) parts.push("attention");
  if (parts.length === 0) return null;
  return parts.join(",");
}

export const eventLeadExtractionEvaluator: EventLeadExtractionEvaluator = {
  extract(context: EventLeadExtractionContext): EventLeadExtractionResult {
    ...
    const decisions: Array<
      | ReturnType<typeof createEventLeadExtractionDecisionLead>
      | ReturnType<typeof createEventLeadExtractionDecisionNotExtracted>
    > = [];
    let index = 0;
    for (const entry of rankedEntries) {
      ...
      if (entry.breakdown.signalDensity === "low" && entry.breakdown.sourceDiversity === "low") {
        reasons.push(
          createEventLeadReason({ code: "insufficient_evidence", label: "Insufficient signal or source diversity", impact: "negative" }),
        );
      ...
      const hypothesisSummary =
        evidenceSet.representativeHeadlineNullable?.trim() ||
        `Cluster ${String(entry.clusterId)}`;
      const lead: EventLead = createEventLead({
