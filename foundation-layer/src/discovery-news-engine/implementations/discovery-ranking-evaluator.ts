...
import type { DiscoveryRankingEvaluator } from "../interfaces/discovery-ranking-evaluator.js";
import type {
  DiscoveryRankingContext,
  DiscoveryRankingResult,
} from "../interfaces/discovery-ranking-evaluator.js";
...
const PRIORITY_ORDER: Record<DiscoveryPriorityClass, number> = {
...
function derivePriorityClass(
  breakdown: DiscoveryRankingEntry["breakdown"],
  reasons: readonly { code: string }[],
  cautionFlags: readonly string[],
  hasAnyTrendingSnapshot: boolean,
  bestSignalCount: number,
): DiscoveryPriorityClass {
  ...
  const strongItalian =
    breakdown.italianRelevance === "high" || breakdown.italianRelevance === "medium";
  const hasAuthorityOrEditorial =
    breakdown.authoritativeRelevance || breakdown.editorialRelevance;
  const enoughSignals =
    breakdown.signalDensity === "high" ||
...
function orderingBasisFor(entry: DiscoveryRankingEntry): (string | number)[] {
  const p = PRIORITY_ORDER[entry.priorityClass];
  const italianOrder =
    entry.breakdown.italianRelevance === "high"
      ? 2
  ...
  const densityOrder =
    entry.breakdown.signalDensity === "high"
      ? 2
...
export const discoveryRankingEvaluator: DiscoveryRankingEvaluator = {
  rank(context: DiscoveryRankingContext): DiscoveryRankingResult {
    ...
    const now = new Date();
    const entries: DiscoveryRankingEntry[] = [];
    ...
    for (const cluster of clusters) {
      ...
      const bestSnapshot = snapshots.length > 0
        ? snapshots.reduce((a, b) =>
            (b.indicatorSet.signalCountInHorizon > a.indicatorSet.signalCountInHorizon ? b : a),
          )
        : null;
      const bestSignalCount = bestSnapshot?.indicatorSet.signalCountInHorizon ?? 0;
      const hasAnyTrendingSnapshot = snapshots.some(
        (s) => s.trendStatus === DiscoveryTrendStatus.TRENDING,
      );

      const priorityClass = derivePriorityClass(
        breakdown,
        reasons,
        cautionFlags,
        hasAnyTrendingSnapshot,
