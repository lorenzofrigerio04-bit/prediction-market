...
export const createRulebookSection = (input: RulebookSection): RulebookSection => {
  ...
  if (!Number.isInteger(input.ordering_index) || input.ordering_index < 0) {
    throw new ValidationError(
      "INVALID_RULEBOOK_SECTION",
      "ordering_index must be an integer greater than or equal to 0",
    );
  }
  return deepFreeze({
    ...input,
