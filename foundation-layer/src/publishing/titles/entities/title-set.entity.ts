...
const validateDisplayCompatibility = (canonicalTitle: string, displayTitle: string): void => {
  if (!isDisplayTitleCompatible(canonicalTitle, displayTitle)) {
    throw new ValidationError(
      "DISPLAY_TITLE_NOT_COMPATIBLE",
      "display_title must be a presentational variant of canonical_title",
    );
  }
};

const validateCanonicalMoreFormal = (canonicalTitle: string, displayTitle: string): void => {
...
const validateSubtitle = (
  ...
  if (forbiddenPatterns.some((pattern) => lowered.includes(pattern))) {
    throw new ValidationError(
      ...
      "subtitle must not introduce new contractual conditions",
    );
  }
  ...
  if (hasNewNumericClaim) {
    throw new ValidationError(
      "SUBTITLE_INTRODUCES_CONDITIONS",
      "subtitle must not add numeric contractual constraints not present upstream",
    );
  }
  return normalized;
};

export const createTitleSet = (input: TitleSet): TitleSet => {
  ...
  return deepFreeze({
    ...
    subtitle,
    generation_metadata: createDeterministicMetadata(
      input.generation_metadata as Record<string, unknown>,
      "generation_metadata",
    ),
  });
};
