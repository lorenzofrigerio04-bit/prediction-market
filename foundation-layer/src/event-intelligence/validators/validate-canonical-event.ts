...
const validateCanonicalEventInvariants = (
  ...
  if (input.supporting_candidates.length === 0) {
    issues.push(
      errorIssue(
        ...
        "supporting_candidates must contain at least one candidate id",
      ),
    );
  }
  if (input.supporting_observations.length === 0) {
    issues.push(
      errorIssue(
        "MISSING_SUPPORTING_OBSERVATIONS",
        "/supporting_observations",
        "supporting_observations must contain at least one observation id",
      ),
    );
  }
  if (new Set(input.supporting_candidates).size !== input.supporting_candidates.length) {
    issues.push(
      errorIssue(
        "DUPLICATE_SUPPORTING_CANDIDATES",
        "/supporting_candidates",
        "supporting_candidates must be unique",
      ),
    );
  }
  if (new Set(input.supporting_observations).size !== input.supporting_observations.length) {
    issues.push(
      errorIssue(
        "DUPLICATE_SUPPORTING_OBSERVATIONS",
        "/supporting_observations",
        "supporting_observations must be unique",
      ),
    );
  }
  if (Date.parse(input.time_window.start_at) > Date.parse(input.time_window.end_at)) {
    issues.push(
      errorIssue("INVALID_TIME_WINDOW", "/time_window", "time_window must satisfy start_at <= end_at"),
    );
  }
  return issues;
};
