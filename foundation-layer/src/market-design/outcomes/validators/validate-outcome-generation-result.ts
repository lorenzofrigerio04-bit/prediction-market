...
import type { OutcomeGenerationResult } from "../entities/outcome-generation-result.entity.js";

const hasRangeOverlap = (input: OutcomeGenerationResult): boolean => {
...
const validateOutcomeGenerationInvariants = (
  input: OutcomeGenerationResult,
  ...
  if (input.contract_type === ContractType.MULTI_OUTCOME && input.outcomes.length < 2) {
    issues.push(
      errorIssue("INVALID_MULTI_OUTCOME_CARDINALITY", "/outcomes", "multi_outcome requires at least 2 outcomes"),
    );
  }
  if (input.contract_type === ContractType.SCALAR_BRACKET) {
    if (input.outcomes.some((outcome) => outcome.range_definition_nullable === null)) {
      issues.push(
        errorIssue(
          "INVALID_SCALAR_RANGE_STRUCTURE",
          "/outcomes",
          "scalar_bracket outcomes require range_definition_nullable",
        ),
      );
    }
    if (hasRangeOverlap(input)) {
      issues.push(
        errorIssue("OVERLAPPING_SCALAR_RANGES", "/outcomes", "scalar_bracket ranges must not overlap"),
      );
    }
  }
  return issues;
...
export const validateOutcomeGenerationResult = (
  input: OutcomeGenerationResult,
