...
import { createOutcomeDefinition, type OutcomeDefinition } from "./outcome-definition.entity.js";
...
const hasOverlaps = (outcomes: readonly OutcomeDefinition[]): boolean => {
...
export const createOutcomeGenerationResult = (
  ...
  if (input.contract_type === ContractType.BINARY && outcomes.length !== 2) {
    throw new ValidationError(
      "INVALID_OUTCOME_GENERATION",
      "binary contract_type requires exactly 2 outcomes",
    );
  }
  if (input.contract_type === ContractType.MULTI_OUTCOME && outcomes.length < 2) {
    throw new ValidationError(
      "INVALID_OUTCOME_GENERATION",
      "multi_outcome contract_type requires at least 2 outcomes",
    );
  }
  if (input.contract_type === ContractType.SCALAR_BRACKET) {
    ...
    if (hasOverlaps(outcomes)) {
      throw new ValidationError(
        "INVALID_OUTCOME_GENERATION",
        "scalar_bracket ranges must not overlap",
      );
    }
  }
  return deepFreeze({
